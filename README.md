...
- app.js CORRECAO RAIZ: nav() ao entrar em Transações forçava reset do período para 'mes' via setPer2 mesmo quando o drawer global (GFLT) tinha outro período ativo (Todos/7d/30d/90d/Ano/Livre) — o filtro escolhido pelo usuário era silenciosamente descartado ao navegar
- app.js CORRECAO RAIZ: renderAll() só re-renderizava a página Resumo, nunca a página visível na tela. Era por isso que parecia 'não atualizar direto' — qualquer save/edit/sync atualizava Resumo em segundo plano mas a página aberta (Transações, Calendário, Parcelas, Orçamento, Grade de Dados) só atualizava ao trocar de página e voltar. Agora renderAll detecta a página ativa via _getActivePage() e re-renderiza ela também
- cfg-migrate.js: _updateRHeroLabel() — label do card hero no Resumo agora é dinâmico ('Total gasto no mês' / 'Total gasto (todos os dados)' / 'Total gasto no ano' etc) conforme o período ativo no GFLT, em vez de ficar sempre fixo em 'no mês' mesmo quando o total exibido já era de outro período
- index.html: label estático 'Total gasto no mês' recebeu id='r-hl' para permitir atualização dinâmica
- app.js: nav() ao entrar em Resumo chama _updateRHeroLabel() para garantir que o label reflita o período já ativo no drawer

### v1.7 — 2026-06-16
**Arquivos:** `app.js`, `index.html`
- index.html: REMOVIDO — aba/página 'Drive' eliminada do menu Config→Dados. App fica mais enxuto com 3 abas (Sync/Importar/CSV) em vez de 4
- index.html: card de Backup automático no Drive movido para dentro da aba Sync, logo abaixo do card Sincronização — sem nenhum campo de URL duplicado, usa cfg.shUrl já configurado no card 'Configurar Apps Script' da mesma aba
- index.html: campo de pasta personalizada do Drive agora dentro de um <details> recolhível — opção secundária, frequência (Manual/Diário/Semanal/Mensal) em destaque
- app.js dadosTab: removida referência à aba 'drv' inexistente — agora itera só sync/imp/csv. updateDriveStatus() chamado ao entrar na aba Sync
- app.js impTab: removida referência órfã a 'drv' — função não tinha uso real no HTML mas foi limpa por consistência
