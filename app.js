...
function addCart(){const nome=document.getElementById('cc-nome').value.trim();if(!nome){toast('Informe o nome.');return}carts.push({id:uid(),nome,cor:document.getElementById('cc-cor').value,band:document.getElementById('cc-band').value,accId:document.getElementById('cc-acc').value,limite:parseMoeda(document.getElementById('cc-lim').value)||0,fech:parseInt(document.getElementById('cc-fech').value)||10,venc:parseInt(document.getElementById('cc-venc').value)||17});saveAll();renderCfgCartoes();closeAddCart();renderCartFlt();populateFlt();populateForm();document.getElementById('cc-nome').value='';toast('Cartão salvo!')}
...
// ── CATEGORIAS ────────────────────────────────────────────────────
function catTab(t,btn){document.getElementById('ct-c').style.display=t==='c'?'block':'none';document.getElementById('ct-s').style.display=t==='s'?'block':'none';document.querySelectorAll('#p-categorias .tab').forEach(b=>b.classList.remove('act'));btn.classList.add('act')}
...
// ── PAGAMENTOS ────────────────────────────────────────────────────
function renderPags(){document.getElementById('pag-list').innerHTML=pags.map(p=>`<div class="item"><span class="bdg ${p.tipo==='credit'||p.tipo==='debit'?'cr':p.tipo==='pix'?'px':p.tipo==='dinheiro'?'dn':'ct'}">${p.tipo}</span><div class="im"><div class="in">${p.nome}</div></div><button class="btn xs dn" onclick="delPag('${p.id}')">${['cr','db','px','dn'].includes(p.id)?'—':'✕'}</button></div>`).join('')}
...
function expJSON(){const data=JSON.stringify({cats,subs,carts,pags,accs,desps:desps.map(d=>({...d,receipt:undefined})),orcs,cfg:{...cfg,avatar:''}},null,2);const a=document.createElement('a');a.href='data:application/json;charset=utf-8,'+encodeURIComponent(data);a.download='financas_v6_backup.json';a.click();toast('Exportado (sem fotos)')}
function impJSON(input){const f=input.files[0];if(!f)return;const r=new FileReader();r.onload=e=>{try{const d=JSON.parse(e.target.result);if(d.desps)desps=d.desps;if(d.cats)cats=d.cats;if(d.subs)subs=d.subs;if(d.carts)carts=d.carts;if(d.pags)pags=d.pags;if(d.accs)accs=d.accs;if(d.orcs)orcs=d.orcs;if(d.cfg){const pin=cfg.pin;const theme=cfg.theme;cfg={...d.cfg,pin,theme}}saveAll();renderAll();toast('Backup importado!')}catch{toast('Arquivo inválido')}};r.readAsText(f)}
...
function toggleTransFilters() {
  ...
  document.getElementById('trans-filter-panel').style.display = transFiltersVisible ? 'block' : 'none';
}
function setPer2(t, el) {
  document.querySelectorAll('#period-chips .chip').forEach(c => c.classList.remove('act'));
  if (el) el.classList.add('act');
  setPer(t, { classList: { add: ()=>{}, remove: ()=>{} } });
}
...
function nav(p, btn) {
  ...
  const inits = {
    ...
    lancar:     () => { populateForm(); },
    transacoes: () => { populateFlt(); renderTrans(); },
...
const _catTabOrig = catTab;
catTab = function(t, btn) {
...
const _populateFormOrig = populateForm;
populateForm = function() {
  _populateFormOrig();
  // Refiltra: só ativos
  const accSel = document.getElementById('l-acc');
  if (accSel) accSel.innerHTML = '<option value="">Selecione...</option>' +
    accs.filter(a=>a.ativa!==false).map(a=>`<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('');
  const cartSel = document.getElementById('l-cart');
  if (cartSel) cartSel.innerHTML = '<option value="">Selecione...</option>' +
    carts.filter(c=>c.ativa!==false).map(c=>`<option value="${c.id}">${c.nome}</option>`).join('');
};

// ── FILTROS MULTI-SELECT ──────────────────────────────────────
// Estado dos filtros ativos
const FLT = {
  accs:  [],  // IDs de contas selecionadas
  carts: [],  // IDs de cartões
  cats:  [],  // IDs de categorias
  subs:  [],  // IDs de subcategorias
  pags:  [],  // IDs de pagamentos
};
...
function renderFltBlock(containerId, items, key, labelFn, onChange) {
  const el = document.getElementById(containerId);
  if (!el) return;
  ...
  el.innerHTML = items.map(item => {
    ...
    return `<div class="flt-chip ${on?'on':''}" onclick="toggleFltChip('${key}','${item.id}',this)">${label}</div>`;
  }).join('');
  // Armazenar o onChange no container
  el.dataset.onChange = onChange.name || '';
}
...
const _renderTransOrig = renderTrans;
renderTrans = function() {
  // Chamar original mas depois refiltar via FLT
  _renderTransOrig();
};
