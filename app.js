...
function renderAll(){
  ...
  // Re-renderizar também a página visível, se for diferente de Resumo —
  // sem isso, ações como salvar/editar/sincronizar só atualizavam o Resumo
  // em segundo plano, e a página aberta na tela ficava com dados antigos
  // até o usuário trocar de página e voltar.
  try {
    ...
    else if (pg === 'orcamento' && typeof renderOrcPlus === 'function') renderOrcPlus();
    else if (pg === 'orcamento' && typeof renderOrc === 'function') renderOrc();
    else if (pg === 'datagrid' && typeof renderDGPlus === 'function') renderDGPlus();
  ...
  } catch(e) { console.warn('renderAll: refresh página ativa falhou', e); }
}
applyTheme();
...
function nav(p, btn) {
  ...
  const inits = {
    ...
    lancar:     () => { populateForm(); },
    transacoes: () => {
      // Só força período "mês" se o drawer global não tiver outro período ativo
      const G = window.GFLT;
      if (!G || G.modo === 'mes') {
        const btn=document.querySelector('#period-chips .chip'); if(btn){setPer2('mes',btn);}
      }
...
// ── FILTRO GLOBAL UNIFICADO (estilo Transações) — V1 ───────
...
function openGlobalFilter(){
  ...
  let panel=document.getElementById('global-flt-panel');
  if(!panel){
    panel=document.createElement('div');
    panel.id='global-flt-panel';
    panel.className='global-filter-inline';
  ...
  const G=window.GFLT;
  const pageName=({resumo:'Resumo',calendario:'Calendário',parcelas:'Parcelamentos',relatorio:'Relatório',datagrid:'Grade de Dados',orcamento:'Orçamento',analise:'Análise'})[page]||page;
  const chip=(label,val)=>`<div class="chip ${G.per===val?'act':''}" onclick="window.GFLT.per='${val}';document.querySelectorAll('#gf-period .chip').forEach(c=>c.classList.remove('act'));this.classList.add('act');document.getElementById('gf-custom').style.display='${val}'==='custom'?'block':'none';">${label}</div>`;
  ...
  panel.innerHTML=`
    <div style="max-width:980px;margin:0 auto;padding:10px 14px 12px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:8px">
        <div>
          <div style="font-size:11px;font-weight:800;color:var(--txt3);text-transform:uppercase;letter-spacing:.06em">Filtros</div>
          <div style="font-size:13px;font-weight:800;color:var(--txt)">${pageName}</div>
        </div>
        <button class="btn xs" onclick="closeGlobalFilter()">Fechar</button>
      </div>
      ...
      <div style="background:var(--bg2);border:0.5px solid var(--bdr);border-radius:var(--r);padding:10px 12px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;gap:8px">
          <span style="font-size:11px;font-weight:700;color:var(--txt3);text-transform:uppercase;letter-spacing:.05em">Marque um ou mais</span>
      ...
      <div style="display:flex;gap:8px;margin-top:10px">
        <button class="btn sm" style="flex:1" onclick="clearGlobalFilter()">Limpar</button>
        <button class="btn pr sm" style="flex:1" onclick="applyGlobalFilter()">Aplicar</button>
      </div>
    </div>`;
...
function closeGlobalFilter(){const p=document.getElementById('global-flt-panel');if(p){p.style.display='none';p.dataset.open='0';}}
