...
let cats=gl('cats')||[];
let subs=gl('subs')||[],carts=gl('carts')||[];
...
// ── IDs defaults legados que NUNCA devem aparecer se vieram só do código ──────
const _DEFAULT_LEGADOS = new Set(['ali','trp','laz','sau','com','out','nu','def']);
const _isDefaultLegado = id => _DEFAULT_LEGADOS.has(id);

// Limpa do localStorage qualquer default legado que não tenha despesas vinculadas
// Sanitiza datas e valores ruins em desps já salvas no localStorage
function _sanitizarDesps() {
  let changed = false;
  desps.forEach(d => {
    const dataLimpa = _parseData(d.data);
    if (d.data !== dataLimpa) { d.data = dataLimpa; changed = true; }
    const valorLimpo = _parseValor(d.valor);
    if (d.valor !== valorLimpo) { d.valor = valorLimpo; changed = true; }
  });
  if (changed) saveAll();
}

function _limparDefaultsLegados() {
  const usedCat  = new Set(desps.map(d=>d.catId).filter(Boolean));
  const usedCart = new Set(desps.map(d=>d.cartId).filter(Boolean));
  const usedAcc  = new Set(desps.map(d=>d.accId).filter(Boolean));
  cats  = cats.filter(c  => !_isDefaultLegado(c.id)  || usedCat.has(c.id));
  carts = carts.filter(c => !_isDefaultLegado(c.id)  || usedCart.has(c.id));
  accs  = accs.filter(a  => !_isDefaultLegado(a.id)  || usedAcc.has(a.id));
  // Subs órfãs (categoria pai removida)
  const catIds = new Set(cats.map(c=>c.id));
  subs = subs.filter(s => catIds.has(s.pai));
  // Limpar metaAccs de IDs que não existem mais em accs
  if (cfg && cfg.metaAccs) {
    const accIds = new Set(accs.map(a=>a.id));
    cfg.metaAccs = cfg.metaAccs.filter(id => accIds.has(id));
  }
}

// ══════════════════════════════════════════════════════════════════
//  _vivos(type) — FONTE ÚNICA DE VERDADE para exibição
//  · Com dados importados: só itens que têm despesas OU _manual=true
//  · App vazio (sem despesas): retorna defaults fictícios para orientar
// ══════════════════════════════════════════════════════════════════
const _DEMO = {
  cats:  [{id:'__d_cat1',nome:'Alimentação',emoji:'🍽',cor:'#6366f1',_demo:true},{id:'__d_cat2',nome:'Transporte',emoji:'🚗',cor:'#14b8a6',_demo:true}],
  subs:  [],
  carts: [{id:'__d_cart1',nome:'Cartão de exemplo',cor:'#8b5cf6',band:'Visa',accId:'__d_acc1',limite:0,fech:10,venc:17,_demo:true}],
  pags:  [{id:'cr',nome:'Crédito',tipo:'credit'},{id:'db',nome:'Débito',tipo:'debit'},{id:'px',nome:'Pix',tipo:'pix'},{id:'dn',nome:'Dinheiro',tipo:'dinheiro'}],
  accs:  [{id:'__d_acc1',nome:'Carteira',emoji:'👛',tipo:'carteira',cor:'#10b981',saldo:0,_demo:true}],
};

function _vivos(type) {
  const _arr = {acc:accs,cart:carts,cat:cats,sub:subs,pag:pags}[type] || [];
  const _campo = {acc:'accId',cart:'cartId',cat:'catId',sub:'subId',pag:'pagId'}[type];
  const temDados = desps.length > 0;

  if (!temDados) {
    // App vazio: mostrar demos para o usuário saber o que configurar
    return _DEMO[type] || [];
  }

  // Com dados: só o que tem despesas vinculadas OU foi criado manualmente
  const usados = new Set(desps.map(d => d[_campo]).filter(Boolean));
  return _arr.filter(item => {
    if (item.ativa === false) return false;
    return usados.has(item.id) || item._manual === true;
  });
}

// Versão para subcategorias filtradas por categoria pai
function _vivosSubs(catIds) {
  const all = _vivos('sub');
  if (!catIds || !catIds.length) return all;
  return all.filter(s => catIds.includes(s.pai));
}
...
const fmtD=d=>{if(!d)return '';const s=String(d);const m=s.match(/^(\d{4})-(\d{2})-(\d{2})/);if(m)return`${m[3]}/${m[2]}/${m[1]}`;return s};
...
function _parseData(raw) {
  ...
  // ISO com T: "2026-06-20T03:00:00.000Z"
  if (/^\d{4}-\d{2}-\d{2}T/.test(raw)) return raw.slice(0,10);
  // Objeto Date serializado pelo Apps Script: "Fri Jun 20 2026 ..."
...
// Converte QUALQUER formato de valor para número
...
function _dedupDesps(){
  const seen=new Set(); const out=[];
  for(const d of desps){ if(d.id && seen.has(d.id)) continue; if(d.id) seen.add(d.id); out.push(d); }
  if(out.length!==desps.length) desps=out;
}
function saveAll(){_dedupDesps();gs('cats',cats);gs('subs',subs);gs('carts',carts);gs('pags',pags);gs('accs',accs);gs('desps',desps);gs('orcs',orcs);gs('pend',pend);gs('cfg',cfg);gs('deletedIds',deletedIds)}
...
let _pkLast={k:'',t:0};
function pk(k){
  // Guard: ignore if same key fired within 80ms (touchstart + onclick double-fire)
  const now=Date.now();
  if(_pkLast.k===k && now-_pkLast.t<80) return;
  _pkLast={k,t:now};
  if(pinBuf.length>=4) return;
  pinBuf+=k; updDots();
  if(pinBuf.length===4) setTimeout(chkPin,130);
}
let _pkDelLast=0;
function pkDel(){
  const now=Date.now(); if(now-_pkDelLast<80) return; _pkDelLast=now;
  if(pinBuf.length>0){pinBuf=pinBuf.slice(0,-1);updDots();}
}
...
function chkPin(){
  if(pinBuf===cfg.pin){
    ...
    // Inicializar app se for primeiro boot
    if(!window._appBooted){ window._appBooted=true; _bootApp(); }
  } else {
    updDots(true);
    document.getElementById('pin-err').textContent='PIN incorreto';
    setTimeout(()=>{ pinBuf=''; updDots(); document.getElementById('pin-err').textContent=''; }, 1000);
  }
}
function lockApp(){
  if(!cfg.lock && !cfg.pin) return;
  window._appBooted = true;
  document.getElementById('shell').style.display='none';
  const lock = document.getElementById('lock');
  lock.classList.add('show');
  lock.style.pointerEvents='all';
  // iOS fix: body overflow:hidden blocks touch on fixed children
  document.body.style.overflow='auto';
  pinBuf=''; updDots(); renderLockProfile();
}
function renderLockProfile(){const box=document.getElementById('lk-av');if(!box)return;const nm=cfg.nome||'Finanças';const lt=document.getElementById('lk-title');if(lt)lt.textContent=nm;box.innerHTML=cfg.avatar&&cfg.avatar.length>10?`<img src="${cfg.avatar}" class="lk-avatar"/>`:`<div class="lk-avatar-init">${nm[0].toUpperCase()}</div>`}
...
function onMes(){
  curMes=document.getElementById('mesSel').value;
  // Sincronizar perStart/perEnd com o mês selecionado
  const [y,mo]=curMes.split('-');
  perStart=curMes+'-01';
  perEnd=new Date(+y,+mo,0).toISOString().slice(0,10);
  // Sincronizar GFLT
  if(window.GFLT){window.GFLT.mes=curMes;if(window.GFLT.modo==='mes'){window.GFLT.d1=perStart;window.GFLT.d2=perEnd;}}
  renderAll();
}
function setPer(t,btn){document.querySelectorAll('.period-btn').forEach(b=>b.classList.remove('act'));btn.classList.add('act');const now=new Date();document.getElementById('custom-range').style.display='none';if(t==='mes'){
  // Usar curMes (mês selecionado), não o mês calendário atual
  const [y,mo]=curMes.split('-');
  perStart=curMes+'-01';
  perEnd=new Date(+y,+mo,0).toISOString().slice(0,10);
}else if(t==='7'){perEnd=today();perStart=new Date(Date.now()-6*864e5).toISOString().slice(0,10)}else if(t==='30'){perEnd=today();perStart=new Date(Date.now()-29*864e5).toISOString().slice(0,10)}else if(t==='90'){perEnd=today();perStart=new Date(Date.now()-89*864e5).toISOString().slice(0,10)}else if(t==='ano'){perStart=now.getFullYear()+'-01-01';perEnd=now.getFullYear()+'-12-31'}else{document.getElementById('custom-range').style.display='block';return}renderTrans()}
function setPerDates(t){const now=new Date();if(t==='mes'){
  // Usar curMes (mês selecionado), não o mês calendário atual
  const [y,mo]=curMes.split('-');
  perStart=curMes+'-01';
  perEnd=new Date(+y,+mo,0).toISOString().slice(0,10);
...
// ── SELECTS ──────────────────────────────────────────────────────
function populateForm(){const mp={pag:'<option value="">Selecione...</option>'+_vivos('pag').map(p=>`<option value="${p.id}">${p.nome}</option>`).join(''),cat:'<option value="">Selecione...</option>'+_vivos('cat').map(c=>`<option value="${c.id}">${c.emoji||''} ${c.nome}</option>`).join(''),cart:'<option value="">Selecione...</option>'+_vivos('cart').map(c=>`<option value="${c.id}">${c.nome}</option>`).join(''),acc:'<option value="">Selecione...</option>'+_vivos('acc').map(a=>`<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('')};Object.entries(mp).forEach(([k,v])=>{const el=document.getElementById('l-'+k);if(el)el.innerHTML=v})}
function populateCatSels(){const np=document.getElementById('ns-p');if(np)np.innerHTML='<option value="">Selecione...</option>'+_vivos('cat').map(c=>`<option value="${c.id}">${c.nome}</option>`).join('')}
function populateParcCatSel(){const el=document.getElementById('parc-cat');if(el)el.innerHTML='<option value="">Todas categorias</option>'+_vivos('cat').map(c=>`<option value="${c.id}">${c.emoji||''} ${c.nome}</option>`).join('')}
function populateRSels(){
  const rc=document.getElementById('r-cat-sel');if(rc)rc.innerHTML='<option value="">Todas categorias</option>'+_vivos('cat').map(c=>`<option value="${c.id}">${c.emoji||''} ${c.nome}</option>`).join('');
  const rs=document.getElementById('r-sub-sel');if(rs)rs.innerHTML='<option value="">Todas subcategorias</option>'+_vivos('sub').map(s=>`<option value="${s.id}">${s.nome}</option>`).join('');
  const ra=document.getElementById('r-acc-sel');if(ra)ra.innerHTML='<option value="">Todas contas</option>'+_vivos('acc').map(a=>`<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('');
  const rp=document.getElementById('r-pag-sel');if(rp)rp.innerHTML='<option value="">Todos pagamentos</option>'+_vivos('pag').map(p=>`<option value="${p.id}">${p.nome}</option>`).join('');
}
function populateCartCCSel(){const el=document.getElementById('cc-acc');if(el)el.innerHTML='<option value="">Nenhuma</option>'+_vivos('acc').map(a=>`<option value="${a.id}">${a.nome}</option>`).join('')}
function populateDelSels(){const mp={'del-cat':'<option value="">Todas categorias</option>'+_vivos('cat').map(c=>`<option value="${c.id}">${c.nome}</option>`).join(''),'del-sub':'<option value="">Todas subcategorias</option>'+_vivos('sub').map(s=>`<option value="${s.id}">${s.nome}</option>`).join(''),'del-card':'<option value="">Todos cartões</option>'+_vivos('cart').map(c=>`<option value="${c.id}">${c.nome}</option>`).join(''),'del-acc':'<option value="">Todas contas</option>'+_vivos('acc').map(a=>`<option value="${a.id}">${a.nome}</option>`).join('')};Object.entries(mp).forEach(([id,html])=>{const el=document.getElementById(id);if(el)el.innerHTML=html})}
function onPag(){
  // Cartão é independente do pagamento — sempre visível
  // Apenas controla se parcelamento faz sentido (só cartão crédito)
  const p=pags.find(x=>x.id===document.getElementById('l-pag').value);
  const isCredit=p&&p.tipo==='credit';
  if(!isCredit){
    const lp=document.getElementById('l-parc'); if(lp)lp.checked=false;
    const lpb=document.getElementById('l-pb'); if(lpb)lpb.style.display='none';
  }
}
function onCat(){const id=document.getElementById('l-cat').value;document.getElementById('l-sub').innerHTML='<option value="">Nenhuma</option>'+_vivosSubs([id]).map(s=>`<option value="${s.id}">${s.nome}</option>`).join('')}
...
function showReceipt(id){const d=desps.find(x=>x.id===id);if(!d?.receipt)return;openModal('Comprovante','',null);document.getElementById('m-btns').style.display='none';document.getElementById('m-extra').innerHTML=`<div style="text-align:center;margin-top:8px"><img src="${d.receipt}" style="max-width:100%;max-height:55vh;width:auto;height:auto;border-radius:10px;border:0.5px solid var(--bdr2);object-fit:contain;display:block;margin:0 auto"/></div><button class="btn sm" style="width:100%;margin-top:11px" onclick="closeModal()">Fechar</button>`}

// ── ADD DESPESA ──────────────────────────────────────────────────
// ── PROTEÇÃO ANTI-DUPLO-CLIQUE ───────────────────────────────────
// Impede que o usuário salve o mesmo formulário várias vezes ao tocar
// rapidamente. Cada form tem sua própria chave de lock.
const _saveLock = {};
function _lock(key, btn) {
  if (_saveLock[key]) return false; // já está salvando
  _saveLock[key] = true;
  if (btn) { btn.disabled = true; btn.dataset._origText = btn.textContent; btn.textContent = '⏳ Salvando...'; }
  return true;
}
function _unlock(key, btn) {
  _saveLock[key] = false;
  if (btn) { btn.disabled = false; btn.textContent = btn.dataset._origText || 'Salvar'; }
}
// Busca o botão principal do form. Tenta pelo ID específico primeiro, depois por onclick.
const _BTN_IDS = {
  addAcc:  'btn-add-acc',
  addPag:  'btn-add-pag',
  addCat:  'btn-add-cat',
  addSub:  'btn-add-sub',
  addDesp: 'btn-add-desp',
};
function _findBtn(fnName) {
  const id = _BTN_IDS[fnName];
  if(id) { const el = document.getElementById(id); if(el) return el; }
  return document.querySelector(`button[onclick*="${fnName}"]`);
}

function addDesp(){const _btn=_findBtn('addDesp');if(!_lock('addDesp',_btn))return;const desc=document.getElementById('l-desc').value.trim(),valor=parseMoeda(document.getElementById('l-val').value),data=document.getElementById('l-date').value||today(),cartId=document.getElementById('l-cart').value,catId=document.getElementById('l-cat').value,subId=document.getElementById('l-sub').value,accId=document.getElementById('l-acc').value,obs=document.getElementById('l-obs').value.trim(),parcel=document.getElementById('l-parc').checked,nparc=parseInt(document.getElementById('l-np').value)||1;let pagId=document.getElementById('l-pag').value;const _cart=carts.find(x=>x.id===cartId);if(_cart){if(_cart.tipoUso==='credit'){const px=pags.find(p=>p.tipo==='credit');if(px)pagId=px.id;}else if(_cart.tipoUso==='debit'){const px=pags.find(p=>p.tipo==='debit');if(px)pagId=px.id;}else{const px=pags.find(p=>p.id===pagId);if(!px||(px.tipo!=='credit'&&px.tipo!=='debit')){_unlock('addDesp',_btn);toast('Cartão dual: selecione Crédito ou Débito.');return;}}}if(!desc||!valor||valor<=0){_unlock('addDesp',_btn);toast('Preencha descrição e valor.');return}if(!catId){_unlock('addDesp',_btn);toast('Selecione a Categoria.');return}if(!pagId){_unlock('addDesp',_btn);toast('Selecione o Pagamento.');return}const newIds=[];if(parcel&&nparc>=2){const pv=valor/nparc,gid=uid();for(let i=1;i<=nparc;i++){const d=new Date(data+'T12:00:00');d.setMonth(d.getMonth()+(i-1));const id=uid();desps.unshift({id,desc,valor:pv,data:d.toISOString().slice(0,10),pagId,cartId,catId,subId,accId,obs,parcela:i,totalParcelas:nparc,grupoId:gid,ss:'p',receipt:i===1?receiptData:null});newIds.push(id)}}else{const id=uid();desps.unshift({id,desc,valor,data,pagId,cartId,catId,subId,accId,obs,ss:'p',receipt:receiptData});newIds.push(id)}pend.push(...newIds);receiptData=null;const prev=document.getElementById('receipt-prev');if(prev){prev.innerHTML='';prev.style.display='none'}saveAll();initMesSel();renderAll();updSyncBnr();['l-desc','l-val','l-obs'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});document.getElementById('l-parc').checked=false;document.getElementById('l-pb').style.display='none';_unlock('addDesp',_btn);toast('✅ Despesa adicionada!');if(cfg.asy!==false&&navigator.onLine)setTimeout(doSync,600)}

// ── EDIT / DEL ───────────────────────────────────────────────────
function editDesp(id){const d=desps.find(x=>x.id===id);if(!d)return;openModal('Editar despesa','',null);document.getElementById('m-btns').style.display='none';const so=_vivosSubs([d.catId]).map(s=>`<option value="${s.id}"${s.id===d.subId?' selected':''}>${s.nome}</option>`).join('');document.getElementById('m-extra').innerHTML=`<div style="display:flex;flex-direction:column;gap:8px"><div class="edit-row"><label>Descrição</label><input id="ed-desc" value="${d.desc}" style="flex:1"/></div><div class="edit-row"><label>Valor R$</label><input id="ed-val" type="number" value="${d.valor.toFixed(2)}" style="width:110px"/></div><div class="edit-row"><label>Data</label><input id="ed-date" type="date" value="${d.data}" style="flex:1"/></div><div class="edit-row"><label>Categoria</label><select id="ed-cat" style="flex:1" onchange="edCatCh()"><option value="">—</option>${_vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}"${c.id===d.catId?' selected':''}>${c.nome}</option>`).join('')}</select></div><div class="edit-row"><label>Subcategoria</label><select id="ed-sub" style="flex:1"><option value="">—</option>${so}</select></div><div class="edit-row"><label>Pagamento</label><select id="ed-pag" style="flex:1"><option value="">—</option>${_vivos('pag').map(p=>`<option value="${p.id}"${p.id===d.pagId?' selected':''}>${p.nome}</option>`).join('')}</select></div><div class="edit-row"><label>Conta</label><select id="ed-acc" style="flex:1"><option value="">—</option>${_vivos('acc').filter(a=>!a._demo).map(a=>`<option value="${a.id}"${a.id===d.accId?' selected':''}>${a.nome}</option>`).join('')}</select></div><div class="edit-row"><label>Obs.</label><input id="ed-obs" value="${d.obs||''}" style="flex:1"/></div>${d.receipt?`<div><img src="${d.receipt}" style="max-width:100%;max-height:110px;border-radius:7px;border:0.5px solid var(--bdr2)"/></div>`:''}${d.totalParcelas?`<div style="font-size:11px;color:var(--txt3)">Parcela ${d.parcela}/${d.totalParcelas} — edita só esta</div>`:''}<div style="display:flex;gap:7px;margin-top:4px"><button class="btn pr sm" style="flex:1" onclick="saveEdit('${id}')">Salvar</button><button class="btn sm" style="flex:1" onclick="closeModal()">Cancelar</button><button class="btn dn sm" onclick="delDesp('${id}');closeModal()">Apagar</button></div></div>`}
...
function saveEdit(id){if(_saveLock['saveDesp'+id])return;_saveLock['saveDesp'+id]=true;const idx=desps.findIndex(x=>x.id===id);if(idx<0){delete _saveLock['saveDesp'+id];return;}desps[idx].desc=document.getElementById('ed-desc').value.trim()||desps[idx].desc;desps[idx].valor=parseMoeda(document.getElementById('ed-val').value)||desps[idx].valor;desps[idx].data=document.getElementById('ed-date').value||desps[idx].data;desps[idx].catId=document.getElementById('ed-cat').value;desps[idx].subId=document.getElementById('ed-sub').value;desps[idx].pagId=document.getElementById('ed-pag').value;desps[idx].accId=document.getElementById('ed-acc').value;desps[idx].obs=document.getElementById('ed-obs').value.trim();desps[idx].ss='p';if(!pend.includes(id))pend.push(id);delete _saveLock['saveDesp'+id];saveAll();closeModal();renderAll();updSyncBnr();if(cfg.asy!==false&&navigator.onLine)setTimeout(doSync,600);toast('✅ Despesa atualizada!')}
function delDesp(id){openModal('Remover despesa?','Esta ação não pode ser desfeita.',async ()=>{
  ...
  if(cfg.shUrl&&navigator.onLine){
    try{
      await fetch(cfg.shUrl,{method:'POST',mode:'no-cors',headers:{'Content-Type':'text/plain'},credentials:'omit',body:JSON.stringify({action:'deleteRow',id})});
    }catch{}
...
function getFiltered(){
  // Fallback: usar curMes quando perStart não está definido
  const [_y,_mo]=curMes.split('-');
  const d1 = perStart || curMes+'-01';
  const d2 = perEnd   || new Date(+_y,+_mo,0).toISOString().slice(0,10);
...
function renderCal(){
  ...
  document.getElementById('cal-title').textContent=new Date(y,m,1).toLocaleDateString('pt-BR',{month:'long',year:'numeric'});
  const fd=new Date(y,m,1).getDay(),dim=new Date(y,m+1,0).getDate();
  const grid=document.getElementById('cal-grid');
  grid.innerHTML='Dom Seg Ter Qua Qui Sex Sáb'.split(' ').map(d=>`<div class="cal-dow">${d}</div>`).join('');
  const ts=today();
  for(let i=0;i<fd;i++) grid.innerHTML+=`<div class="cal-day other"></div>`;
  for(let day=1;day<=dim;day++){
    const ds=`${y}-${String(m+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    // Incluir despesas reais + parcelas futuras na mesma data
    const dd=desps.filter(d=>d.data===ds);
    const iT=ds===ts, iS=ds===calSel, iF=ds>ts;
    // Dots: até 5, futuras com opacidade menor
    const dots=dd.slice(0,5).map(d=>{
      const c=cats.find(x=>x.id===d.catId);
      const isFut=d.data>ts;
      return `<div class="cal-dot" style="background:${c?c.cor:'#888'};${isFut?'opacity:.35':''}${d.totalParcelas?';border:1px solid rgba(255,255,255,.3)':''}"></div>`;
    }).join('');
    // Total do dia (incluindo futuras)
    const dayTot=dd.reduce((s,d)=>s+d.valor,0);
    const totStr=dayTot>0?(dayTot>=1000?(dayTot/1000).toFixed(1)+'k':dayTot.toFixed(0)):'';
    grid.innerHTML+=`<div class="cal-day${iT?' today':''}${iS?' sel':''}${iF?' fut':''}" data-date="${ds}" onclick="calClick('${ds}')">
      <span class="cal-num">${day}</span>
      <div class="cal-dots">${dots}</div>
      ${totStr?`<div class="cal-day-val">${totStr}</div>`:''}
    </div>`;
  }
  renderCalDetail();
}
...
function renderCalDetail(){
  ...
  const ts=today();
  const iF=calSel>ts;
  const dd=desps.filter(d=>d.data===calSel);
  const pagas=dd.filter(d=>!d.totalParcelas||d.data<=ts);
  const futuras=dd.filter(d=>d.totalParcelas&&d.data>ts);
  const tot=dd.reduce((s,d)=>s+d.valor,0);
  let html=`<div class="cal-detail"><div class="cdh">
    <span class="cdt">${fmtD(calSel)}${iF?' <span class="bdg" style="font-size:9px;background:var(--ind)">futura</span>':''}</span>
    <span class="cda">${fmt(tot)}</span>
  </div>`;
  if(!dd.length){
    html+='<div class="empty">Sem lançamentos</div>';
  } else {
    if(pagas.length) html+=pagas.map(x=>despHTML(x)).join('');
    if(futuras.length){
      html+=`<div style="font-size:10px;font-weight:700;color:var(--txt3);text-transform:uppercase;letter-spacing:.06em;padding:8px 0 4px">Parcelas futuras</div>`;
      html+=futuras.map(x=>`${despHTML(x)}`).join('');
    }
  }
  html+='</div>';
  el.innerHTML=html;
  ...
  openModal('Remover parcelamento?','Todas as parcelas deste parcelamento serão removidas.',async ()=>{
    ...
    if(cfg.shUrl&&navigator.onLine){
      for(const id of toDelete){
        try{await fetch(cfg.shUrl,{method:'POST',headers:{'Content-Type':'text/plain'},credentials:'omit',body:JSON.stringify({action:'deleteRow',id})})}catch{}
      }
      await saveAppData();
    }
...
// ── CONTAS ────────────────────────────────────────────────────────
function renderAccSummary(){const el=document.getElementById('acc-summary');if(!el)return;const vAccs=_vivos('acc').filter(a=>!a._demo);const total=vAccs.reduce((s,a)=>s+(a.saldo||0),0);el.innerHTML=`<div class="hero" style="margin-bottom:10px"><div class="hl">Saldo total</div><div class="hv" style="color:${total>=0?'#fff':'var(--rose)'}">${fmt(total)}</div><div class="hs">${vAccs.length} conta${vAccs.length!==1?'s':''}</div></div>`}
function renderAccList(){document.getElementById('acc-list').innerHTML=_vivos('acc').filter(a=>!a._demo).length?_vivos('acc').filter(a=>!a._demo).map(a=>{const g=desps.filter(d=>d.accId===a.id&&mesKey(d.data)===curMes).reduce((s,d)=>s+d.valor,0);return`<div class="acc-card"><div class="acc-icon" style="background:${a.cor}22">${a.emoji||'🏦'}</div><div style="flex:1;min-width:0"><div class="acc-name">${a.nome}</div><div class="acc-sub">${a.tipo} · gasto ${fmt(g)} este mês</div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px"><div class="acc-bal ${a.saldo>=0?'pos':'neg'}">${fmt(a.saldo)}</div><div style="display:flex;gap:3px"><button class="btn xs" onclick="editAcc('${a.id}')">✏️</button><button class="btn xs dn" onclick="delAcc('${a.id}')">✕</button></div></div></div>`}).join(''):'<div class="empty">Nenhuma conta</div>'}
function addAcc(){const _btn=_findBtn('addAcc');if(!_lock('addAcc',_btn))return;const nome=document.getElementById('ac-nome').value.trim();if(!nome){_unlock('addAcc',_btn);toast('Informe o nome.');return}accs.push({id:uid(),nome,emoji:document.getElementById('ac-emoji').value.trim()||'🏦',tipo:document.getElementById('ac-tipo').value,cor:document.getElementById('ac-cor').value,saldo:parseMoeda(document.getElementById('ac-saldo').value)||0,_manual:true});saveAll();renderAccSummary();renderCfgContas();closeAddAcc();populateFlt();populateForm();populateCartCCSel();['ac-nome','ac-emoji','ac-saldo'].forEach(id=>{const el=document.getElementById(id);if(el)el.value=''});_unlock('addAcc',_btn);toast('✅ Conta adicionada!')}
function editAcc(id){
  const a=accs.find(x=>x.id===id); if(!a) return;
  // Fechar outros painéis abertos
  document.querySelectorAll('.cfg-edit-panel.open').forEach(p=>p.classList.remove('open'));
  let panel = document.getElementById('edit-acc-'+id);
  if(!panel){
    panel = document.createElement('div');
    panel.id = 'edit-acc-'+id;
    panel.className = 'cfg-edit-panel';
    panel.innerHTML = `
      <h4>✏ Editar Conta</h4>
      <div class="frow"><div class="fg"><label>Nome</label><input id="ea-n-${id}" value="${a.nome.replace(/"/g,'&quot;')}"/></div>
        <div class="fg" style="max-width:70px"><label>Emoji</label><input id="ea-e-${id}" value="${a.emoji||''}" maxlength="2" style="text-align:center"/></div></div>
      <div class="frow"><div class="fg"><label>Saldo R$</label><input id="ea-s-${id}" type="number" value="${a.saldo}" step="0.01"/></div>
        <div class="fg" style="max-width:60px"><label>Cor</label><input id="ea-c-${id}" type="color" value="${a.cor||'#6366f1'}" style="height:44px"/></div></div>
      <div style="display:flex;gap:8px;margin-top:4px">
        <button class="btn pr sm" style="flex:1" onclick="saveAccEdit('${id}')">✅ Salvar</button>
        <button class="btn sm" onclick="this.closest('.cfg-edit-panel').classList.remove('open')">Cancelar</button>
      </div>`;
    // Inserir após o cfg-item correspondente
    const items = document.querySelectorAll('#acc-list .cfg-item');
    const target = Array.from(items).find(el=>el.querySelector(`button[onclick*="'${id}'"]`));
    if(target) target.after(panel);
    else document.getElementById('acc-list').appendChild(panel);
  }
  panel.classList.add('open');
  panel.scrollIntoView({behavior:'smooth',block:'nearest'});
  const ni = document.getElementById('ea-n-'+id); if(ni) ni.focus();
}
function saveAccEdit(id){
  if(_saveLock['saveAcc'+id])return; _saveLock['saveAcc'+id]=true;
  const idx=accs.findIndex(x=>x.id===id);
  if(idx<0){delete _saveLock['saveAcc'+id];return;}
  const n=document.getElementById('ea-n-'+id)||document.getElementById('ea-n');
  const e=document.getElementById('ea-e-'+id)||document.getElementById('ea-e');
  const s=document.getElementById('ea-s-'+id)||document.getElementById('ea-s');
  const c=document.getElementById('ea-c-'+id)||document.getElementById('ea-c');
  if(n) accs[idx].nome=n.value.trim()||accs[idx].nome;
  if(e) accs[idx].emoji=e.value.trim();
  if(s) accs[idx].saldo=parseMoeda(s.value)||0;
  if(c) accs[idx].cor=c.value;
  saveAll();
  const panel=document.getElementById('edit-acc-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  renderCfgContas();populateFlt();populateForm();renderAccSummary();renderAccList();
  delete _saveLock['saveAcc'+id];
  if(cfg.shUrl&&navigator.onLine)saveAppData();
  toast('✅ Conta atualizada!');
}
function delAcc(id){openModal('Remover conta?','',()=>{accs=accs.filter(a=>a.id!==id);saveAll();renderAccSummary();renderAccList();renderCfgContas();if(cfg.shUrl&&navigator.onLine)saveAppData();})}

// ── CARTÕES + FILTRO ──────────────────────────────────────────────
function renderCartFlt(){const cs=document.getElementById('cf-card');if(cs)cs.innerHTML='<option value="">Todos os cartões</option>'+_vivos('cart').map(c=>`<option value="${c.id}">${c.nome}</option>`).join('');const cm=document.getElementById('cf-mes');if(cm){const ms=[...new Set([today().slice(0,7),...desps.map(d=>mesKey(d.data))])].sort().reverse();cm.innerHTML='<option value="">Este mês</option>'+ms.map(m=>{const[y,mo]=m.split('-');const n=new Date(y,+mo-1,1).toLocaleDateString('pt-BR',{month:'short',year:'numeric'});return`<option value="${m}"${m===curMes?' selected':''}>${n}</option>`}).join('')}renderCartList()}
...
function addCart(){const _btn=document.getElementById('cc-save-btn');if(!_lock('addCart',_btn))return;const nome=document.getElementById('cc-nome').value.trim();if(!nome){_unlock('addCart',_btn);toast('Informe o nome.');return}const tipoEl=document.getElementById('cc-tipo'),tipoUso=tipoEl?tipoEl.value:'ambos';const corEl=document.getElementById('cc-cor');const fechEl=document.getElementById('cc-fec')||document.getElementById('cc-fech');const vencEl=document.getElementById('cc-vec')||document.getElementById('cc-venc');const editId=tipoEl&&tipoEl.dataset.editId;if(editId){const idx=carts.findIndex(c=>c.id===editId);if(idx>=0){carts[idx]={...carts[idx],nome,cor:corEl?corEl.value:carts[idx].cor,band:document.getElementById('cc-band').value,accId:document.getElementById('cc-acc').value,limite:parseMoeda(document.getElementById('cc-lim').value)||0,fech:parseInt(fechEl?fechEl.value:'')||10,venc:parseInt(vencEl?vencEl.value:'')||17,tipoUso};}}else{carts.push({id:uid(),nome,cor:corEl?corEl.value:'#6366f1',band:document.getElementById('cc-band').value,accId:document.getElementById('cc-acc').value,limite:parseMoeda(document.getElementById('cc-lim').value)||0,fech:parseInt(fechEl?fechEl.value:'')||10,venc:parseInt(vencEl?vencEl.value:'')||17,tipoUso,ativa:true,_manual:true});}saveAll();if(typeof renderCfgCartoes==='function')renderCfgCartoes();if(typeof renderCartList==='function')renderCartList();closeAddCart();if(typeof renderCartFlt==='function')renderCartFlt();if(typeof populateFlt==='function')populateFlt();populateForm();document.getElementById('cc-nome').value='';if(tipoEl)tipoEl.dataset.editId='';const btn=document.getElementById('cc-save-btn');if(btn)btn.textContent='Salvar';_unlock('addCart',document.getElementById('cc-save-btn'));if(cfg.shUrl&&navigator.onLine)saveAppData();toast(editId?'✅ Cartão atualizado!':'✅ Cartão salvo!');}
function editCart(id){
  const c=carts.find(x=>x.id===id); if(!c) return;
  document.querySelectorAll('.cfg-edit-panel.open').forEach(p=>p.classList.remove('open'));
  let panel=document.getElementById('edit-cart-'+id);
  if(!panel){
    const accOpts='<option value="">Nenhuma</option>'+_vivos('acc').filter(a=>!a._demo).map(a=>`<option value="${a.id}"${a.id===c.accId?' selected':''}>${a.emoji||'🏦'} ${a.nome}</option>`).join('');
    panel=document.createElement('div');
    panel.id='edit-cart-'+id;
    panel.className='cfg-edit-panel';
    panel.innerHTML=`
      <h4>✏ Editar Cartão</h4>
      <div class="frow">
        <div class="fg"><label>Nome</label><input id="ec-n-${id}" value="${c.nome.replace(/"/g,'&quot;')}"/></div>
        <div class="fg" style="max-width:60px"><label>Cor</label><input id="ec-cor-${id}" type="color" value="${c.cor||'#6366f1'}" style="height:44px"/></div>
      </div>
      <div class="frow">
        <div class="fg"><label>Limite R$</label><input id="ec-l-${id}" type="number" value="${c.limite||0}" step="0.01"/></div>
        <div class="fg" style="max-width:80px"><label>Fecha dia</label><input id="ec-f-${id}" type="number" value="${c.fech||10}" min="1" max="28"/></div>
        <div class="fg" style="max-width:80px"><label>Vence dia</label><input id="ec-v-${id}" type="number" value="${c.venc||17}" min="1" max="28"/></div>
      </div>
      <div class="frow"><div class="fg"><label>Conta vinculada</label><select id="ec-acc-${id}">${accOpts}</select></div></div>
      <div style="display:flex;gap:8px;margin-top:4px">
        <button class="btn pr sm" style="flex:1" onclick="saveCartEdit('${id}')">✅ Salvar</button>
        <button class="btn sm" onclick="this.closest('.cfg-edit-panel').classList.remove('open')">Cancelar</button>
      </div>`;
    const items=document.querySelectorAll('#cart-list .cfg-item');
    const target=Array.from(items).find(el=>el.querySelector(`button[onclick*="'${id}'"]`));
    if(target) target.after(panel); else document.getElementById('cart-list').appendChild(panel);
  }
  panel.classList.add('open');
  panel.scrollIntoView({behavior:'smooth',block:'nearest'});
}
function saveCartEdit(id){
  if(_saveLock['saveCart'+id])return; _saveLock['saveCart'+id]=true;
  const idx=carts.findIndex(x=>x.id===id);
  if(idx<0){delete _saveLock['saveCart'+id];return;}
  const get=s=>document.getElementById(s+id)||document.getElementById(s.replace('-'+id,''));
  const n=get('ec-n-');if(n) carts[idx].nome=n.value.trim()||carts[idx].nome;
  const l=get('ec-l-');if(l) carts[idx].limite=parseMoeda(l.value)||0;
  const f=get('ec-f-');if(f) carts[idx].fech=parseInt(f.value)||carts[idx].fech;
  const v=get('ec-v-');if(v) carts[idx].venc=parseInt(v.value)||carts[idx].venc;
  const a=get('ec-acc-');if(a) carts[idx].accId=a.value;
  const cr=get('ec-cor-');if(cr) carts[idx].cor=cr.value;
  saveAll();
  const panel=document.getElementById('edit-cart-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  renderCfgCartoes();populateFlt();populateForm();renderCartFlt();
  delete _saveLock['saveCart'+id];
  if(cfg.shUrl&&navigator.onLine)saveAppData();
  toast('✅ Cartão atualizado!');
}
function delCart(id){openModal('Remover cartão?','',()=>{carts=carts.filter(c=>c.id!==id);saveAll();renderCartFlt();renderCfgCartoes();if(cfg.shUrl&&navigator.onLine)saveAppData();})}

// ── CATEGORIAS ────────────────────────────────────────────────────

function renderCats(){document.getElementById('cat-list').innerHTML=_vivos('cat').filter(c=>!c._demo).length?_vivos('cat').filter(c=>!c._demo).map(c=>`<div class="item"><div class="swatch" style="background:${c.cor}"></div><div class="im"><div class="in">${c.emoji||''} ${c.nome}</div><div class="is">${subs.filter(s=>s.pai===c.id).map(s=>s.nome).join(', ')||'sem subcategorias'}</div></div><div style="display:flex;gap:3px"><button class="btn xs" onclick="editCat('${c.id}')">✏️</button><button class="btn xs dn" onclick="delCat('${c.id}')">✕</button></div></div>`).join(''):'<div class="empty">Nenhuma categoria</div>';document.getElementById('sub-list').innerHTML=_vivos('sub').length?_vivos('sub').map(s=>{const p=cats.find(c=>c.id===s.pai);return`<div class="item"><div class="im"><div class="in">${s.nome}</div><div class="is">${p?p.nome:'—'}</div></div><div style="display:flex;gap:3px"><button class="btn xs" onclick="editSub('${s.id}')">✏️</button><button class="btn xs dn" onclick="delSub('${s.id}')">✕</button></div></div>`}).join(''):'<div class="empty">Nenhuma subcategoria</div>'}
function addCat(){const _btn=_findBtn('addCat');if(!_lock('addCat',_btn))return;const nome=document.getElementById('nc-n').value.trim();if(!nome){_unlock('addCat',_btn);toast('Informe o nome.');return}cats.push({id:uid(),nome,cor:document.getElementById('nc-c').value,emoji:document.getElementById('nc-e').value.trim(),_manual:true});saveAll();renderCfgCats();closeAddCat();populateFlt();populateForm();populateCatSels();renderCats();document.getElementById('nc-n').value='';document.getElementById('nc-e').value='';_unlock('addCat',_btn);if(cfg.shUrl&&navigator.onLine)saveAppData();toast('✅ Categoria adicionada!')}
function editCat(id){
  const c=cats.find(x=>x.id===id); if(!c) return;
  document.querySelectorAll('.cfg-edit-panel.open').forEach(p=>p.classList.remove('open'));
  let panel=document.getElementById('edit-cat-'+id);
  if(!panel){
    panel=document.createElement('div');
    panel.id='edit-cat-'+id;
    panel.className='cfg-edit-panel';
    panel.innerHTML=`
      <h4>✏ Editar Categoria</h4>
      <div class="frow">
        <div class="fg"><label>Nome</label><input id="ec2-n-${id}" value="${c.nome.replace(/"/g,'&quot;')}"/></div>
        <div class="fg" style="max-width:70px"><label>Emoji</label><input id="ec2-e-${id}" value="${c.emoji||''}" maxlength="2" style="text-align:center"/></div>
        <div class="fg" style="max-width:60px"><label>Cor</label><input id="ec2-c-${id}" type="color" value="${c.cor||'#6366f1'}" style="height:44px"/></div>
      </div>
      <div style="display:flex;gap:8px;margin-top:4px">
        <button class="btn pr sm" style="flex:1" onclick="saveCatEdit('${id}')">✅ Salvar</button>
        <button class="btn sm" onclick="this.closest('.cfg-edit-panel').classList.remove('open')">Cancelar</button>
      </div>`;
    const items=document.querySelectorAll('#cat-list .cfg-item');
    const target=Array.from(items).find(el=>el.querySelector(`button[onclick*="'${id}'"]`));
    if(target) target.after(panel); else document.getElementById('cat-list').appendChild(panel);
  }
  panel.classList.add('open');
  panel.scrollIntoView({behavior:'smooth',block:'nearest'});
}
function saveCatEdit(id){
  if(_saveLock['saveCat'+id])return; _saveLock['saveCat'+id]=true;
  const idx=cats.findIndex(x=>x.id===id);
  if(idx<0){delete _saveLock['saveCat'+id];return;}
  const n=document.getElementById('ec2-n-'+id)||document.getElementById('ec2-n');
  const e=document.getElementById('ec2-e-'+id)||document.getElementById('ec2-e');
  const c=document.getElementById('ec2-c-'+id)||document.getElementById('ec2-c');
  if(n) cats[idx].nome=n.value.trim()||cats[idx].nome;
  if(e) cats[idx].emoji=e.value.trim();
  if(c) cats[idx].cor=c.value;
  saveAll();
  const panel=document.getElementById('edit-cat-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  renderCfgCats();populateFlt();populateForm();renderCats();
  delete _saveLock['saveCat'+id];
  if(cfg.shUrl&&navigator.onLine)saveAppData();
  toast('✅ Categoria atualizada!');
}
function delCat(id){openModal('Remover?','Subcategorias também removidas.',()=>{cats=cats.filter(c=>c.id!==id);subs=subs.filter(s=>s.pai!==id);saveAll();populateCatSels();renderCats();renderCfgCats();if(cfg.shUrl&&navigator.onLine)saveAppData();})}
function addSub(){const _btn=_findBtn('addSub');if(!_lock('addSub',_btn))return;const pai=document.getElementById('ns-p').value,nome=document.getElementById('ns-n').value.trim();if(!pai||!nome){_unlock('addSub',_btn);toast('Selecione categoria e informe o nome.');return}subs.push({id:uid(),pai,nome,_manual:true});saveAll();renderCfgCats();closeAddSub();populateFlt();populateForm();populateCatSels();renderCats();document.getElementById('ns-n').value='';_unlock('addSub',_btn);if(cfg.shUrl&&navigator.onLine)saveAppData();toast('✅ Subcategoria adicionada!')}
function editSub(id){
  const s=subs.find(x=>x.id===id); if(!s) return;
  document.querySelectorAll('.cfg-edit-panel.open').forEach(p=>p.classList.remove('open'));
  let panel=document.getElementById('edit-sub-'+id);
  if(!panel){
    const catOpts='<option value="">—</option>'+_vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}"${c.id===s.pai?' selected':''}>${c.emoji||''} ${c.nome}</option>`).join('');
    panel=document.createElement('div');
    panel.id='edit-sub-'+id;
    panel.className='cfg-edit-panel';
    panel.innerHTML=`
      <h4>✏ Editar Subcategoria</h4>
      <div class="frow"><div class="fg"><label>Nome</label><input id="es-n-${id}" value="${s.nome.replace(/"/g,'&quot;')}"/></div></div>
      <div class="frow"><div class="fg"><label>Categoria pai</label><select id="es-p-${id}">${catOpts}</select></div></div>
      <div style="display:flex;gap:8px;margin-top:4px">
        <button class="btn pr sm" style="flex:1" onclick="saveSubEdit('${id}')">✅ Salvar</button>
        <button class="btn sm" onclick="this.closest('.cfg-edit-panel').classList.remove('open')">Cancelar</button>
      </div>`;
    const items=document.querySelectorAll('#sub-list .cfg-item');
    const target=Array.from(items).find(el=>el.querySelector(`button[onclick*="'${id}'"]`));
    if(target) target.after(panel); else document.getElementById('sub-list').appendChild(panel);
  }
  panel.classList.add('open');
  panel.scrollIntoView({behavior:'smooth',block:'nearest'});
}
function saveSubEdit(id){
  if(_saveLock['saveSub'+id])return; _saveLock['saveSub'+id]=true;
  const idx=subs.findIndex(x=>x.id===id);
  if(idx<0){delete _saveLock['saveSub'+id];return;}
  const n=document.getElementById('es-n-'+id)||document.getElementById('es-n');
  const p=document.getElementById('es-p-'+id)||document.getElementById('es-p');
  if(n) subs[idx].nome=n.value.trim()||subs[idx].nome;
  if(p) subs[idx].pai=p.value||subs[idx].pai;
  saveAll();
  const panel=document.getElementById('edit-sub-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  renderCfgCats();populateFlt();populateForm();renderCats();
  delete _saveLock['saveSub'+id];
  if(cfg.shUrl&&navigator.onLine)saveAppData();
  toast('✅ Subcategoria atualizada!');
}
function delSub(id){subs=subs.filter(s=>s.id!==id);saveAll();renderCats();renderCfgCats();if(cfg.shUrl&&navigator.onLine)saveAppData();}
...
function addPag(){const _btn=_findBtn('addPag');if(!_lock('addPag',_btn))return;const nome=document.getElementById('np-n').value.trim();if(!nome){_unlock('addPag',_btn);toast('Informe o nome.');return}pags.push({id:uid(),nome,tipo:document.getElementById('np-t').value,_manual:true});saveAll();renderCfgPags();closeAddPag();populateFlt();populateForm();renderPags();document.getElementById('np-n').value='';_unlock('addPag',_btn);if(cfg.shUrl&&navigator.onLine)saveAppData();toast('✅ Pagamento adicionado!')}
function delPag(id){if(['cr','db','px','dn'].includes(id)){toast('Não é possível remover os tipos padrão.');return}pags=pags.filter(p=>p.id!==id);saveAll();renderPags();renderCfgPags();if(cfg.shUrl&&navigator.onLine)saveAppData();}
...
async function testSync(){
  if(!cfg.shUrl){toast('Cole a URL do Web App primeiro.');return}
  if(!navigator.onLine){toast('Sem internet.');return}
  syncUI('sync','Testando...');
  const st  = document.getElementById('sh-url-status');
  const det = document.getElementById('sh-sheet-detected');
  const su  = document.getElementById('sh-sheet-url');
  const hora = () => new Date().toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});
  try{
    const ctrl = new AbortController();
    setTimeout(() => ctrl.abort(), 12000);
    const res  = await fetch(cfg.shUrl+'?action=info', { signal: ctrl.signal });
    const json = await res.json();
    if(json && json.ok){
      syncUI('on','Conectado ✓');
      // Sempre salvar sheetUrl quando disponível
      if(json.sheetUrl) {
        cfg.sheetUrl = json.sheetUrl;
        if(su)  su.value = json.sheetUrl;
        if(det) det.style.display = 'flex';
        saveAll();
        renderSyncSt();
      } else if(cfg.sheetUrl && det) {
        det.style.display = 'flex';
      }
      const sheetMsg = json.sheetUrl ? ' · Planilha vinculada ✓' : (cfg.sheetUrl ? ' · Planilha já vinculada ✓' : '');
      if(st) st.innerHTML = `<span style="color:var(--eme)">✓ Conectado às ${hora()}${sheetMsg}</span>`;
      toast('✓ Conexão confirmada!' + (json.sheetUrl ? ' Planilha detectada.' : ''));
    } else {
      syncUI('off','Erro');
      if(st) st.innerHTML = `<span style="color:var(--rose)">✗ ${json.error||'Erro no script — reimplante o Apps Script v7.4'}</span>`;
    }
  } catch(e) {
    try{
      const ctrl2 = new AbortController();
      setTimeout(() => ctrl2.abort(), 8000);
      await fetch(cfg.shUrl+'?action=ping', {mode:'no-cors', credentials:'omit', signal: ctrl2.signal});
      syncUI('on','URL ok');
      if(st) st.innerHTML = `<span style="color:var(--amb)">⚡ URL alcançada · faça um lançamento para confirmar</span>`;
      toast('URL alcançada. Faça um lançamento para confirmar.');
      // Tentar pegar sheetUrl via JSONP (sem CORS)
      try {
        const sUrl = await _fetchSheetUrlJSONP(cfg.shUrl);
        if(sUrl){ cfg.sheetUrl=sUrl; saveAll(); renderSyncSt();
          const su2=document.getElementById('sh-sheet-url'); if(su2) su2.value=sUrl;
          const det2=document.getElementById('sh-sheet-detected'); if(det2) det2.style.display='flex';
        }
      } catch(e2){}
    } catch {
      syncUI('off','Falha');
      if(st) st.innerHTML = `<span style="color:var(--rose)">✗ URL não alcançada. Verifique se publicou com "Qualquer pessoa".</span>`;
      toast('URL não alcançada. Verifique a implantação.');
    }
  }
}
// ── INPUT AO VIVO: valida URL e busca planilha automaticamente ──
let _shUrlDebounce = null;
function onShUrlInput(val) {
  clearTimeout(_shUrlDebounce);
  const st  = document.getElementById('sh-url-status');
  const det = document.getElementById('sh-sheet-detected');
  const v   = (val || '').trim();
  if (!v) {
  ...
  if (!v.includes('script.google.com')) {
    if (st)  st.innerHTML = '<span style="color:var(--rose)">✗ Cole a URL do Web App (script.google.com/macros/...)</span>';
    if (det) det.style.display = 'none';
    return;
  }
  if (st) st.innerHTML = '<span style="color:var(--amb)">⏳ Detectando planilha...</span>';
  _shUrlDebounce = setTimeout(() => _autoDetectSheet(v), 800);
}

function onFirstRunUrlInput(val) {
  const st = document.getElementById('first-run-status');
  const v  = (val||'').trim();
  if (!st) return;
  if (!v) { st.innerHTML = ''; return; }
  if (!v.includes('script.google.com')) {
    st.innerHTML = '<span style="color:var(--rose)">✗ URL inválida — deve ser script.google.com/macros/...</span>';
  } else {
    st.innerHTML = '<span style="color:var(--amb)">⏳ Verificando...</span>';
    clearTimeout(_shUrlDebounce);
    _shUrlDebounce = setTimeout(() => _autoDetectSheet(v, 'first-run-status'), 800);
  }
}

// Tenta buscar a URL da planilha a partir da URL do Web App.
// Funciona com e sem CORS — quando CORS bloqueia a resposta, usa um iframe trick
// ou guarda a URL para verificar ao salvar.
async function _autoDetectSheet(url, statusId='sh-url-status') {
  const st  = document.getElementById(statusId);
  const det = document.getElementById('sh-sheet-detected');
  const su  = document.getElementById('sh-sheet-url');
  const setStatus = (html) => { if (st) st.innerHTML = html; };

  // Tentativa 1: fetch direto — funciona quando CORS está liberado (script autorizado)
  try {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 10000);
    const res  = await fetch(url + '?action=info', { signal: ctrl.signal });
    clearTimeout(timer);
    const json = await res.json();
    if (json && json.ok) {
      if (json.sheetUrl) {
        ...
        if (su)  su.value = json.sheetUrl;
        if (det) det.style.display = 'flex';
        setStatus('<span style="color:var(--eme)">✓ Web App conectado · Planilha detectada automaticamente ✓</span>');
      } else {
        setStatus('<span style="color:var(--eme)">✓ Web App conectado</span>');
      }
      return; // sucesso total
    }
    setStatus('<span style="color:var(--rose)">✗ Script retornou erro — reimplante o Apps Script v7.4.</span>');
    return;
  } catch(e1) {
    // CORS bloqueou leitura da resposta — tentar ping no-cors para confirmar que URL existe
    try {
      const ctrl2 = new AbortController();
      setTimeout(() => ctrl2.abort(), 8000);
      await fetch(url + '?action=ping', { mode:'no-cors', signal: ctrl2.signal });
      // URL existe mas CORS impede leitura — URL é válida, planilha será buscada ao salvar
      setStatus('<span style="color:var(--amb)">⚡ URL alcançada · planilha será detectada ao salvar</span>');
      if (det) det.style.display = 'none';
    } catch {
      setStatus('<span style="color:var(--rose)">✗ URL não alcançada. Verifique se implantou como "Qualquer pessoa".</span>');
    }
  }
}

async function saveSyncCfg() {
  const shUrlInput = ((document.getElementById('sh-url') || {}).value || '').trim();
  if (shUrlInput) cfg.shUrl = shUrlInput;
  if (!cfg.shUrl) { toast('Cole a URL do Web App primeiro.'); return; }
  ...
  // Sempre tentar buscar a URL da planilha — mesmo que já tenhamos, confirmar que ainda é válida
  setStatusSaving(st, '⏳ Conectando e detectando planilha...');
  let sheetDetected = false;
  try {
    const ctrl  = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 12000);
    const r = await fetch(cfg.shUrl + '?action=info', { signal: ctrl.signal });
    clearTimeout(timer);
    const j = await r.json();
    if (j && j.ok) {
      if (j.sheetUrl) {
        cfg.sheetUrl = j.sheetUrl;
        if (su) su.value = j.sheetUrl;
        sheetDetected = true;
      }
    }
  } catch(e) {
    // CORS bloqueia leitura — tentar JSONP para obter sheetUrl sem CORS
    try {
      const ctrl2 = new AbortController();
      setTimeout(() => ctrl2.abort(), 8000);
      await fetch(cfg.shUrl + '?action=ping', { mode:'no-cors', credentials:'omit', signal: ctrl2.signal });
      // URL existe! Tentar JSONP para sheetUrl
      try {
        const sUrl = await _fetchSheetUrlJSONP(cfg.shUrl);
        if(sUrl){ cfg.sheetUrl = sUrl; if(su) su.value = sUrl; sheetDetected = true; }
      } catch(je){ sheetDetected = !!cfg.sheetUrl; }
    } catch {
      setStatusSaving(st, '<span style="color:var(--rose)">✗ URL não alcançada. Verifique a implantação.</span>');
      toast('URL não alcançada. Verifique se publicou com "Qualquer pessoa".');
      return;
    }
  }
  ...
  if (b && cfg.shUrl) b.style.display = 'none';

  if (sheetDetected) {
    if (det) det.style.display = 'flex';
    if (st)  st.innerHTML = '<span style="color:var(--eme)">✓ Conectado · Planilha vinculada automaticamente ✓</span>';
    toast('✓ Conectado! Planilha detectada.');
  } else {
    if (det) det.style.display = 'none';
    if (st)  st.innerHTML = '<span style="color:var(--amb)">✓ URL salva · faça um lançamento para confirmar</span>';
    toast('✓ URL salva. Faça um lançamento para confirmar a sync.');
  }

  renderSyncSt(); updateSyncHero();
}

function setStatusSaving(el, msg) {
  if (el) el.innerHTML = msg;
}
async function openSheetDirect(){
  // Se já temos a URL da planilha, abrir direto
  if(cfg.sheetUrl){ window.open(cfg.sheetUrl,'_blank'); return; }
  if(!cfg.shUrl){ toast('Configure a URL do Web App primeiro.'); return; }

  // Tentar obter sheetUrl via JSONP (sem CORS) — carrega script dinâmico
  toast('🔎 Detectando planilha...');
  try {
    const sheetUrl = await _fetchSheetUrlJSONP(cfg.shUrl);
    if(sheetUrl){
      cfg.sheetUrl = sheetUrl;
      saveAll();
      const su=document.getElementById('sh-sheet-url'); if(su) su.value=sheetUrl;
      const det=document.getElementById('sh-sheet-detected'); if(det) det.style.display='flex';
      window.open(sheetUrl,'_blank');
      return;
    }
  } catch(e){}

  // Fallback final: mostrar diálogo informativo
  toast('Abra a planilha manualmente no Google Sheets.');
  window.open('https://docs.google.com/spreadsheets/', '_blank');
}

// Busca sheetUrl via JSONP — funciona mesmo com CORS bloqueado
function _fetchSheetUrlJSONP(scriptUrl) {
  return new Promise((resolve, reject) => {
    ...
    window[cbName] = function(data){
      ...
      delete window[cbName];
      if(script.parentNode) script.parentNode.removeChild(script);
      if(data && data.ok && data.sheetUrl) resolve(data.sheetUrl);
      else reject(new Error('no sheetUrl'));
    };
    const script = document.createElement('script');
    script.src = scriptUrl + '?action=info&callback=' + cbName;
...
function renderSyncSt(){
  ...
  if(!el)return;
  const cnt=pend.length,on=navigator.onLine;
  const sheetTag=cfg.sheetUrl
    ? `<button class="btn xs" onclick="openSheetDirect()" style="margin-left:6px;font-size:10px">↗ Abrir Planilha</button>`
    : '';
  const scriptSt=cfg.shUrl
    ? `<div style="font-size:11px;color:var(--eme);padding:6px 10px;background:rgba(16,185,129,.1);border-radius:var(--rs);margin-top:7px;display:flex;align-items:center;flex-wrap:wrap;gap:4px">✓ Web App configurado${cfg.sheetUrl?' · Planilha vinculada ✓':''}${sheetTag}</div>`
    : '<div style="font-size:12px;color:var(--amb);padding:8px 10px;background:rgba(245,158,11,.1);border-radius:var(--rs);margin-top:7px">⚠ Configure a URL do Web App abaixo para ativar a sync.</div>';
  el.innerHTML=`<div style="display:flex;align-items:center;gap:9px;padding:10px;background:var(--bg3);border-radius:var(--rs)"><div style="width:9px;height:9px;border-radius:50%;background:${on?'var(--eme)':'var(--rose)'}"></div><div><div style="font-size:13px;font-weight:600">${on?'Online':'Sem internet'}</div><div style="font-size:11px;color:var(--txt3);margin-top:1px">${cnt>0?cnt+' pendente(s) aguardando sync':'Tudo sincronizado ✓'}</div></div></div>${scriptSt}`;
}
...
function savePerfil(){cfg.nome=document.getElementById('pf-nome').value.trim();cfg.email=document.getElementById('pf-email').value.trim();cfg.meta=parseMoeda(document.getElementById('pf-meta').value)||0;saveAll();renderSBAvatar();document.getElementById('sb-name').textContent=cfg.nome||'Eu';renderLockProfile();if(typeof renderMetaAccs==='function')renderMetaAccs();if(typeof updateMetaBar==='function')updateMetaBar();if(cfg.shUrl&&navigator.onLine)saveAppData();toast('Perfil salvo!')}
...
function expJSON(){const data=JSON.stringify({cats,subs,carts,pags,accs,desps:desps.map(d=>({...d,receipt:undefined})),orcs,cfg:{...cfg,avatar:''},scriptUrl:cfg.shUrl||'',url_sistema:cfg.shUrl||''},null,2);const a=document.createElement('a');a.href='data:application/json;charset=utf-8,'+encodeURIComponent(data);a.download='financas_v1_backup.json';a.click();toast('Exportado (sem fotos)')}
...
// ── INIT ──────────────────────────────────────────────────────────
function renderAll(){
  initMesSel();renderResumo();renderBadge();updSyncBnr();
  try {
    const pg = typeof _getActivePage==='function' ? _getActivePage() : 'resumo';
    if(pg==='transacoes'  && typeof renderTrans==='function')      renderTrans();
    else if(pg==='calendario' && typeof renderCalPlus==='function') renderCalPlus();
    else if(pg==='calendario' && typeof renderCal==='function')     renderCal();
    else if(pg==='parcelas'   && typeof renderParcs==='function')   renderParcs();
    else if(pg==='orcamento'  && typeof renderOrc==='function')     renderOrc();
    else if(pg==='datagrid'   && typeof renderDG==='function')      renderDG();
    else if(pg==='relatorio'  && typeof buildRelatorio==='function') buildRelatorio();
    else if(pg==='perfil'     && typeof renderMetaAccs==='function') renderMetaAccs();
  } catch(e){}
}
applyTheme();
...
async function saveAppData() {
  ...
  // Usar no-cors diretamente: Google Apps Script recebe o POST mas o browser bloqueia
  // a leitura da resposta (CORS). Os dados CHEGAM normalmente ao script.
  try {
    await fetch(cfg.shUrl, {
      method:'POST', mode:'no-cors',
      credentials:'omit',
      headers:{'Content-Type':'text/plain'},
      body: JSON.stringify({ action:'saveAppData', payload })
    });
    return true; // no-cors: não temos resposta, mas dados foram enviados
  } catch(err) {
    console.warn('saveAppData falhou:', err);
    return false;
  }
}
...
// Constrói cats/accs/carts/pags/subs dos valores únicos das colunas.
// Nunca usa defaults — só o que existe na planilha.
async function loadDespsFromSheet() {
  ...
  try {
    ...
    // Passo 1: coletar valores únicos das colunas
    const colCat=new Set(), colSub=new Set(), colPag=new Set(), colCart=new Set(), colAcc=new Set();
    json.rows.forEach(row => {
      if (row[2]) colCat.add(String(row[2]).trim());
      if (row[3]) colSub.add(String(row[3]).trim());
      if (row[4]) colPag.add(String(row[4]).trim());
      if (row[5]) colCart.add(String(row[5]).trim());
      if (row[6]) colAcc.add(String(row[6]).trim());
    });

    // Helper: busca pelo nome (case-insensitive) ou cria
    const _ensure = (arr, nome, extras) => {
      const ex = arr.find(x => x.nome.toLowerCase() === nome.toLowerCase());
      if (ex) return ex.id;
      const item = { id: uid(), nome, ...extras };
      arr.push(item);
      return item.id;
    };

    // Passo 2: zerar arrays — reconstruir 100% a partir da planilha
    // Remove qualquer default legado que possa estar no localStorage
    _limparDefaultsLegados();
    // Reinicia arrays limpos para reconstrução
    cats  = cats.filter(c  => !_isDefaultLegado(c.id));
    carts = carts.filter(c => !_isDefaultLegado(c.id));
    accs  = accs.filter(a  => !_isDefaultLegado(a.id));

    // Passo 3: garantir um item para cada valor único das colunas
    const CORES = ['#6366f1','#14b8a6','#8b5cf6','#f43f5e','#f59e0b','#10b981','#3b82f6','#ec4899','#84cc16','#f97316'];
    const EMOJIS = ['📦','🚗','🎮','💊','🛍','🏠','📱','✈️','🍽','💡'];
    let ci = 0;
    colCat.forEach(nome => _ensure(cats, nome, { cor: CORES[ci%CORES.length], emoji: EMOJIS[ci++%EMOJIS.length] }));
    colSub.forEach(nome => {
      if (!subs.find(s => s.nome.toLowerCase() === nome.toLowerCase()))
        subs.push({ id: uid(), nome, pai: cats[0]?.id || '' });
    });
    const PAG_TIPO = { 'crédito':'credit','credito':'credit','débito':'debit','debito':'debit','pix':'pix','dinheiro':'dinheiro','cash':'dinheiro' };
    colPag.forEach(nome  => _ensure(pags,  nome, { tipo: PAG_TIPO[nome.toLowerCase()] || 'outro' }));
    colCart.forEach(nome => _ensure(carts, nome, { cor:'#6366f1', band:'Outro', accId:'', limite:0, fech:10, venc:17, ativa:true }));
    colAcc.forEach(nome  => _ensure(accs,  nome, { emoji:'🏦', tipo:'carteira', cor:'#6366f1', saldo:0 }));

    // Passo 4: importar despesas com IDs corretos
    let imported = 0;
    json.rows.forEach(row => {
      if (!row[11]) return;
      const id = String(row[11]);
      if (desps.find(d => d.id === id)) return;
      if (deletedIds.includes(id)) return;
      const n = s => String(s||'').trim().toLowerCase();
      const catObj  = cats.find(c  => c.nome.toLowerCase()  === n(row[2]));
      const subObj  = subs.find(s  => s.nome.toLowerCase()  === n(row[3]));
      const pagObj  = pags.find(p  => p.nome.toLowerCase()  === n(row[4]));
      const cartObj = carts.find(c => c.nome.toLowerCase()  === n(row[5]));
      const accObj  = accs.find(a  => a.nome.toLowerCase()  === n(row[6]));
      desps.push({
        id, desc: String(row[1]||''), data: _parseData(row[0]),
        catId:  catObj  ? catObj.id  : '',
        subId:  subObj  ? subObj.id  : '',
        pagId:  pagObj  ? pagObj.id  : '',
        cartId: cartObj ? cartObj.id : '',
        accId:  accObj  ? accObj.id  : '',
        valor: _parseValor(row[7]),
        parcela: row[8]?parseInt(row[8]):null, totalParcelas: row[9]?parseInt(row[9]):null,
        obs: String(row[10]||''), ss:'s', grupoId: id
      });
      imported++;
    });
    if (imported > 0 || colCat.size > 0) {
      saveAll();
      // Atualizar selects e filtros com os dados reais da planilha
    ...
    return imported;
  } catch(e) { console.warn('loadDespsFromSheet:', e); return 0; }
}
...
async function loadAllFromCloud(urlParam, silent=false) {
  ...
  const temAppData = data && Object.keys(data).length > 0;

  if (temAppData) {
    // Tem AppData — restaurar configurações
    ...
    // Zerar arrays ANTES para não mesclar com defaults locais
    ...
    const PAGS_CORE = [{id:'cr',nome:'Crédito',tipo:'credit'},{id:'db',nome:'Débito',tipo:'debit'},{id:'px',nome:'Pix',tipo:'pix'},{id:'dn',nome:'Dinheiro',tipo:'dinheiro'}];
    const pagsNuvem = Array.isArray(data.pags) ? data.pags : [];
    const coreIds   = new Set(PAGS_CORE.map(p=>p.id));
    pags = [...PAGS_CORE, ...pagsNuvem.filter(p => !coreIds.has(p.id))];
    orcs = data.orcs || orcs;
    if (data.deletedIds) deletedIds = data.deletedIds;
    if (data.avatar && data.avatar.length > 10 && !cfg.avatar) cfg.avatar = data.avatar;
    if (data.cfg_profile) {
    ...
    // Sem AppData ainda — ok, continuar para loadDespsFromSheet
    // que reconstrói cats/accs/carts/pags/subs direto das colunas
    console.log('loadAllFromCloud: AppData vazio, carregando direto das linhas');
  }

  _limparDefaultsLegados();
  saveAll();

  // Sempre carrega as despesas da planilha independente de ter AppData ou não
  const imported = await loadDespsFromSheet();

  // Após carregar, populateFlt e populateForm com dados reais
  initMesSel(); renderAll(); renderSBAvatar(); renderLockProfile();
  populateForm(); populateFlt();
  document.getElementById('sb-name').textContent = cfg.nome || 'Eu';
  applyTheme();

  localStorage.setItem('fin6_cloud_loaded','1');
  syncUI('on', 'Sincronizado ✓');
  const n = imported > 0 ? `${imported} despesa(s)` : 'configurações';
  const msg = temAppData
    ? `✓ Dados carregados! ${imported} despesa(s) importada(s).`
    : `✓ ${imported} despesa(s) importada(s) da planilha.`;
  toast(msg);
...
async function autoLoadOnOpen() {
  ...
  // Usar URL do config.js se localStorage não tiver
  ...
  const isNovo = desps.length === 0;

  if (isNovo) {
    // Dispositivo novo — carregar AUTOMATICAMENTE sem perguntar
    // Mostrar banner de progresso não-bloqueante
    _showLoadBanner('Carregando dados da nuvem...');
    syncUI('sync', 'Carregando...');
    updateSyncHero();
  ...
  // Já tem dados — buscar só despesas novas da planilha silenciosamente
  const imported = await loadDespsFromSheet();
  if (imported > 0) {
...
// Banner não-bloqueante para mostrar progresso de carga
function _showLoadBanner(msg) {
  let b = document.getElementById('_load-banner');
  if (!b) {
    b = document.createElement('div');
    b.id = '_load-banner';
    b.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:var(--ind);color:#fff;font-size:13px;font-weight:600;padding:10px 16px;display:flex;align-items:center;gap:10px;box-shadow:0 2px 16px rgba(0,0,0,.3)';
    document.body.appendChild(b);
  }
  b.innerHTML = `<div style="width:14px;height:14px;border:2px solid rgba(255,255,255,.4);border-top-color:#fff;border-radius:50%;animation:spin .7s linear infinite;flex-shrink:0"></div><span>${msg}</span>`;
  b.style.display = 'flex';
}
function _hideLoadBanner() {
  const b = document.getElementById('_load-banner');
...
function toggleTransFilters() {
  ...
  if (panel) {
    panel.style.display = transFiltersVisible ? 'block' : 'none';
    if (transFiltersVisible) {
      populateFlt();  // sempre atualiza os chips ao abrir
      setTimeout(() => panel.scrollIntoView({behavior:'smooth', block:'nearest'}), 50);
    }
  }
  // Indicador visual no botão de filtros
  const btn = document.querySelector('[onclick="toggleTransFilters()"]');
  if (btn) btn.style.color = transFiltersVisible ? 'var(--accent2)' : '';
}
...
async function captureAndShare() {
  ...
  ctx.fillText('Finanças Pessoais V1 · ' + new Date().toLocaleDateString('pt-BR'), 24, fy + FOOTER / 2 + 5);
...
function populateDGCatSel() {
  ...
  el.innerHTML = '<option value="">Todas categorias</option>' + _vivos('cat').filter(c=>!c._demo).map(c => `<option value="${c.id}">${c.nome}</option>`).join('');
}
...
function nav(p, btn) {
  ...
  // Inicializar página
  const inits = {
    resumo:     () => { populateRSels(); renderResumo(); if(typeof _updateRHeroLabel==='function')_updateRHeroLabel(); },
    lancar:     () => { populateForm(); },
    transacoes: () => {
      const G=window.GFLT;
      if(!G||G.modo==='mes'){const btn=document.querySelector('#period-chips .chip');if(btn)setPer2('mes',btn);}
      populateFlt(); renderTrans();
    },
    ...
    config:     () => { if(typeof cfgTypeTab==='function'){const t=document.querySelector('.cfg-type-tab');cfgTypeTab('contas',t);}else{cfgTab('contas',null);} },
...
// ── GAS POST — envia para Apps Script via no-cors ──────────────
async function gasPost(url, body) {
  if (!url || !navigator.onLine) throw new Error('offline');
  await fetch(url, {
...
async function doSync() {
  if (!cfg.shUrl) { console.warn('doSync: no shUrl'); return; }
  if (!navigator.onLine) { console.warn('doSync: offline'); return; }

  // Auto-detectar URL da planilha se ainda não temos
  if (!cfg.sheetUrl) {
    try {
      const ctrl = new AbortController();
  ...
  for (let i = 0; i < toSync.length; i += BATCH) {
    ...
    try {
      // Envia via no-cors — dados chegam mesmo sem ler resposta (CORS)
      // Google Apps Script recebe o POST e processa normalmente
      await fetch(cfg.shUrl, {
        method: 'POST',
        mode: 'no-cors',
        credentials: 'omit',
      ...
      if (fetchErr.name === 'AbortError') {
        toast(`⏱ Timeout. Toque Enviar para continuar.`);
        break;
      }
      toast('Erro de rede. Verifique a conexão e tente novamente.');
      break;
    }
  ...
  syncUI(pend.length===0 ? 'on' : 'off',
         pend.length===0 ? `Sync ${hora}` : `${pend.length} pendente(s)`);
  if (done > 0) {
    toast(`✅ ${done} registro(s) enviado(s) à planilha!`);
    // Tentar obter sheetUrl se ainda não temos (via JSONP)
    if (!cfg.sheetUrl) {
      _fetchSheetUrlJSONP(cfg.shUrl).then(url => {
        if(url){ cfg.sheetUrl=url; saveAll(); renderSyncSt();
          const su=document.getElementById('sh-sheet-url'); if(su) su.value=url;
          const det=document.getElementById('sh-sheet-detected'); if(det) det.style.display='flex';
        }
      }).catch(()=>{});
    }
...
function _initCalFltSels() {
  ...
  if (calAcc) calAcc.innerHTML = '<option value="">Todas contas</option>'+
    _vivos('acc').filter(a=>!a._demo).map(a=>`<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('');
  const calCat = document.getElementById('cal-cat-flt');
  if (calCat) calCat.innerHTML = '<option value="">Todas cats</option>'+
    _vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}">${c.emoji||''} ${c.nome}</option>`).join('');
}
...
async function backupToDrive(silent) {
  if (!cfg.shUrl) {
    if (!silent) { toast('Configure a URL do Apps Script em Planilha/Sync primeiro.'); nav('planilha', null); }
    return;
  }
  if (!navigator.onLine) { if (!silent) toast('Sem internet.'); return; }

  // Elementos só existem se o usuário estiver na tela de Drive — ok não ter
  ...
  if (!silent) toast('Iniciando backup...');
  ...
  try {
    // Envia via no-cors — dados chegam ao script mesmo sem leitura da resposta (CORS)
    await fetch(cfg.shUrl, {
      method: 'POST',
      mode: 'no-cors',
      credentials: 'omit',
      ...
      body: JSON.stringify({
        ...
        rows,
        meta: { cats, subs, accs, folder: cfg.driveFolder || '', filename: `Financas_${today()}.xlsx` }
      })
    });
    // Com no-cors não lemos resposta — assumir sucesso e pedir ao usuário para verificar
    cfg.lastDriveBackup = new Date().toISOString();
    saveAll();
    if (res) res.innerHTML = `<span style="color:var(--eme)">✓ Backup enviado! Verifique seu Google Drive em instantes.</span>`;
    if (dot) dot.style.background = 'var(--eme)';
    updateDriveStatus();
    toast(silent ? '✓ Backup automático enviado ao Drive!' : '✓ Backup enviado ao Drive!');
  } catch(e) {
    console.error('[backupToDrive] erro:', e);
    if (res) res.innerHTML = `<span style="color:var(--rose)">✗ Erro ao enviar: verifique a URL e a conexão.</span>`;
    if (dot) dot.style.background = 'var(--rose)';
    if (!silent) toast('Erro ao enviar backup.');
  }
}
...
function checkAutoBackup() {
  if (!cfg.shUrl) { console.log('[autoBackup] sem URL configurada'); return; }
  if (!navigator.onLine) { console.log('[autoBackup] offline, pulando'); return; }
  const freq = cfg.driveFreq || 'never';
  if (freq === 'never') { console.log('[autoBackup] frequência = manual, não roda'); return; }
  ...
  const THRESHOLDS = {
    daily:   23  * 3600 * 1000,        // ~23h
    weekly:  6.5 * 24 * 3600 * 1000,   // ~6.5 dias
    monthly: 28  * 24 * 3600 * 1000,   // ~28 dias
  };
  const threshold = THRESHOLDS[freq] || THRESHOLDS.weekly;

  console.log('[autoBackup] freq='+freq+' último='+(last?last.toISOString():'nunca')+' diff(h)='+(diff/3600000).toFixed(1)+' threshold(h)='+(threshold/3600000).toFixed(1));

  if (diff > threshold) {
    console.log('[autoBackup] disparando backup automático ('+freq+')...');
    setTimeout(() => backupToDrive(true), 4000);
  }
}
...
function editAssin(id) {
  ...
  const cOpts = '<option value="">Nenhuma</option>' + _vivos('cat').filter(c=>!c._demo).map(c =>
    `<option value="${c.id}"${c.id===r.catId?' selected':''}>${c.emoji||''} ${c.nome}</option>`).join('');
  const aOpts = '<option value="">Nenhuma</option>' + _vivos('acc').filter(a=>!a._demo).map(a =>
...
function initRelatorio() {
  ...
  if(rc) rc.innerHTML = '<option value="">Todas</option>' + _vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}">${c.emoji||''} ${c.nome}</option>`).join('');
  // Contas
  const ra = document.getElementById('rel-acc');
  if(ra) ra.innerHTML = '<option value="">Todas</option>' + _vivos('acc').filter(a=>!a._demo).map(a=>`<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('');
  // Pag
  const rp = document.getElementById('rel-pag');
  if(rp) rp.innerHTML = '<option value="">Todos</option>' + _vivos('pag').map(p=>`<option value="${p.id}">${p.nome}</option>`).join('');
  buildRelatorio();
}
...
function populateAssinSelects() {
  ...
  if (aa) aa.innerHTML = '<option value="">Nenhuma</option>' + _vivos('acc').filter(a=>!a._demo).map(a => `<option value="${a.id}">${a.emoji||'🏦'} ${a.nome}</option>`).join('');
}

function populateDel2Sels() {
  const dc = document.getElementById('del2-cat');
  if (dc) dc.innerHTML = '<option value="">Todas cats</option>' + _vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}">${c.nome}</option>`).join('');
...
function renderCalPlus() {
  ...
  // Mini-total já está embutido no renderCal via .cal-day-val
  // Atualizar com filtros de acc/cat se ativos
  if (fAcc || fCat) {
    document.querySelectorAll('.cal-day').forEach(cell => {
      ...
      const existing = cell.querySelector('.cal-day-val');
...
function renderDGPlus() {
  ...
  if(dc&&dc.options.length<=1) dc.innerHTML='<option value="">Todas cats</option>'+_vivos('cat').filter(c=>!c._demo).map(c=>`<option value="${c.id}">${c.nome}</option>`).join('');
...
function renderMetaAccs() {
  ...
  // Só contas reais — que têm despesas vinculadas (não demos, não legados)
  const vAccs = _vivos('acc').filter(a => !a._demo);

  // Limpar cfg.metaAccs de IDs que não existem mais nas contas reais
  cfg.metaAccs = cfg.metaAccs.filter(id => vAccs.some(a => a.id === id));
  saveAll(); // persistir limpeza imediatamente

  if (!vAccs.length) {
    el.innerHTML = '<div style="font-size:12px;color:var(--txt3);padding:8px 0">Nenhuma conta encontrada na planilha.</div>';
    return;
  }

  el.innerHTML = vAccs.map(a => {
...
function renderMigCats() {
  ...
  el.innerHTML = _vivos('cat').filter(c=>!c._demo).map(c => {
...
function saveDriveCfg() {
  ...
  cfg.driveFreq   = document.querySelector('input[name="drive-freq"]:checked')?.value || 'never';
  saveAll();
  updateDriveStatus();
  const res = document.getElementById('drive-result');
  if (res) {
    const labels = {never:'Manual (só quando você tocar em Backup agora)', daily:'Diário', weekly:'Semanal', monthly:'Mensal'};
    res.innerHTML = `<span style="color:var(--txt3)">Frequência definida: ${labels[cfg.driveFreq]||cfg.driveFreq}</span>`;
  }
}
...
function updateDriveStatus() {
  ...
  const FREQ_LABEL = {never:'Manual', daily:'Diário', weekly:'Semanal', monthly:'Mensal'};
  const freq = cfg.driveFreq || 'never';
  if (title) title.textContent = cfg.shUrl ? ('Backup ' + (FREQ_LABEL[freq]||freq)) : 'Não configurado';
  if (last) {
    ...
    dot.style.background = cfg.shUrl ? 'var(--amb)' : 'var(--txt3)';
    text.textContent = cfg.shUrl ? 'Nenhum backup realizado ainda' : 'Configure o script primeiro';
  }
...
// ── BOOT ──────────────────────────────────────────────────────
function _bootApp() {
  _sanitizarDesps();      // corrigir datas/valores ruins do localStorage
  _limparDefaultsLegados(); // remover defaults hardcoded sem uso
  if (cfg.shUrl && cats.length === 0 && carts.length === 0 && accs.length === 0) {
    // App tem URL mas arrays vazios — não poluir UI com defaults
    // renderAll vai mostrar telas vazias até a nuvem responder
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Mostrar tela de bloqueio sempre que PIN estiver configurado
  // (não depende do toggle "Bloquear ao minimizar" — esse controla só o minimize)
  ...
  // ── Bloquear ao minimizar / sair do app ─────────────────
  // Ativa a tela de bloqueio quando o app vai para background,
  // se o usuário tiver ativado "Bloquear ao minimizar"
  let _lockTimer = null;
  document.addEventListener('visibilitychange', () => {
    if(!cfg.pin || cfg.pin.length !== 4) return;
    if(document.hidden) {
      // App foi minimizado ou aba ficou inativa
      if(cfg.lock) {
        // Pequeno delay para evitar travar durante transições normais (ex: copiar texto)
        _lockTimer = setTimeout(() => {
          if(document.hidden) lockApp();
        }, 300);
      }
    } else {
      // App voltou ao foco — cancelar lock se ainda não aconteceu
      clearTimeout(_lockTimer);
    }
  });

  // Fallback para iOS Safari que às vezes não dispara visibilitychange
  window.addEventListener('pagehide', () => {
    if(cfg.lock && cfg.pin && cfg.pin.length === 4) lockApp();
  });
});

// ══════════════════════════════════════════════════════════════
//  V1 — FIXES: boot, sync dispositivo novo, primeiro acesso
// ══════════════════════════════════════════════════════════════
...
async function firstRunLoad() {
  ...
  if (!url || !url.includes('script.google.com')) {
    if (st) st.innerHTML = '<span style="color:var(--rose)">✗ URL inválida — cole a URL completa do Web App</span>';
    return;
  }
  if (!navigator.onLine) {
  ...
  if (st) st.innerHTML = '<span style="color:var(--amb)">⏳ Conectando ao Web App...</span>';

  // 1. Tentar buscar ?action=info (GET — retorna sheetUrl se CORS ok)
  let urlOk = false;
  try {
    const ctrl  = new AbortController();
    setTimeout(() => ctrl.abort(), 12000);
    ...
    if (json && json.ok) {
      ...
      if (json.sheetUrl) {
        cfg.sheetUrl = json.sheetUrl;
        const su  = document.getElementById('sh-sheet-url');  if (su)  su.value  = json.sheetUrl;
        const det = document.getElementById('sh-sheet-detected'); if (det) det.style.display = 'flex';
        if (st) st.innerHTML = '<span style="color:var(--eme)">✓ Web App conectado · Planilha detectada ✓</span>';
      } else {
        if (st) st.innerHTML = '<span style="color:var(--eme)">✓ Web App conectado</span>';
      }
    } else {
      if (st) st.innerHTML = '<span style="color:var(--rose)">✗ Script retornou erro — reimplante o Apps Script v7.4.</span>';
      return;
    }
  } catch(e) {
    // CORS bloqueou — tentar ping no-cors para confirmar existência da URL
    try {
      const ctrl2 = new AbortController();
      setTimeout(() => ctrl2.abort(), 8000);
      await fetch(url + '?action=ping', { mode:'no-cors', signal: ctrl2.signal });
      urlOk = true;
      if (st) st.innerHTML = '<span style="color:var(--amb)">⚡ URL alcançada — carregando dados...</span>';
    } catch {
      if (st) st.innerHTML = '<span style="color:var(--rose)">✗ URL não alcançada. Verifique se implantou com "Qualquer pessoa".</span>';
      return;
    }
  }
  ...
  // 2. Salvar URL
  cfg.shUrl = url;
  const uEl = document.getElementById('sh-url'); if (uEl) uEl.value = url;
  saveAll();
  if (st) st.innerHTML = '<span style="color:var(--amb)">⏳ Carregando configurações e despesas da planilha...</span>';
  updateSyncHero();

  // 3. Carregar tudo da nuvem
  await loadAllFromCloud(url, true);

  const banner = document.getElementById('first-run-banner');
  if (banner) banner.style.display = 'none';
  if (st) st.innerHTML = cfg.sheetUrl
    ? '<span style="color:var(--eme)">✓ Conectado com sucesso! Planilha vinculada automaticamente.</span>'
    : '<span style="color:var(--eme)">✓ Conectado! Faça um lançamento para confirmar a sync.</span>';
  initMesSel(); renderAll(); updateSyncHero();
  toast('✓ App conectado à planilha!');
}
...
// ══════════════════════════════════════════════════════════════
//  V1 — CONFIG UNIFICADO + DADOS + FILTROS MULTI-SELECT
// ══════════════════════════════════════════════════════════════
...
function renderCfgContas() {
  ...
  if (!el) return;
  // Só mostra contas que existem nas despesas (vêm da planilha)
  const lista = accs.filter(a => {
    const usado = desps.some(d => d.accId === a.id);
    return usado || a._manual === true;  // _manual: criado pelo usuário nesta sessão
  });
  if (!lista.length) { el.innerHTML = '<div class="empty">Nenhuma conta encontrada na planilha.</div>'; return; }
  el.innerHTML = lista.map((a,i) => {
    ...
    return `<div class="cfg-item ${ativo?'':'cfg-item-off'}">
      ...
      <div class="cfg-item-actions">
        <button class="btn xs" onclick="editAcc('${a.id}')">✏ Editar</button>
        <button class="btn xs dn" onclick="delAcc('${a.id}')">✕</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderCfgCartoes() {
  ...
  if (!el) return;
  // Só mostra cartões que existem nas despesas (vêm da planilha)
  const lista = carts.filter(c => {
    const usado = desps.some(d => d.cartId === c.id);
    return usado || c._manual === true;
  });
  if (!lista.length) { el.innerHTML = '<div class="empty">Nenhum cartão encontrado na planilha.</div>'; return; }
  el.innerHTML = lista.map((c,i) => {
    ...
    return `<div class="cfg-item ${ativo?'':'cfg-item-off'}">
      ...
      <div class="cfg-item-actions">
        <button class="btn xs" onclick="editCart('${c.id}')">✏ Editar</button>
        <button class="btn xs dn" onclick="delCart('${c.id}')">✕</button>
      </div>
    </div>`;
  }).join('');
}
...
function renderCfgPags() {
  ...
  // Só mostra pagamentos com despesas vinculadas (os 4 core: cr/db/px/dn só aparecem se usados)
  const CORE = new Set(['cr','db','px','dn']);
  const lista = pags.filter(p => {
    const usado = desps.some(d => d.pagId === p.id);
    return usado || p._manual === true;
  });
  if (!lista.length) { el.innerHTML = '<div class="empty">Nenhum pagamento encontrado.</div>'; return; }
    ...
    <div class="cfg-item">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div style="display:flex;gap:4px">
          <button class="btn xs" onclick="editPag('${p.id}')">✏ Editar</button>
          <button class="btn xs dn" onclick="delPag('${p.id}')">✕</button>
        </div>
      </div>
    </div>`).join('');
}
...
function renderCfgCats() {
  ...
  // Só mostra categorias/subs que existem nas despesas
  const catsLista = cats.filter(c => desps.some(d => d.catId === c.id) || c._manual === true);
  const subsLista = subs.filter(s => desps.some(d => d.subId === s.id) || s._manual === true);
  if (catEl) catEl.innerHTML = catsLista.length ? catsLista.map((c,i) => {
    ...
    return `<div class="cfg-item">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <button class="btn xs" onclick="editCat('${c.id}')">✏</button>
        <button class="btn xs dn" onclick="delCat('${c.id}')">✕</button>
      </div>
    </div>`;
  }).join('') : '<div class="empty">Nenhuma categoria encontrada na planilha.</div>';

  if (subEl) subEl.innerHTML = subsLista.length ? subsLista.map((s,i) => {
    ...
    return `<div class="cfg-item">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <button class="btn xs" onclick="editSub('${s.id}')">✏</button>
        <button class="btn xs dn" onclick="delSub('${s.id}')">✕</button>
      </div>
    </div>`;
  }).join('') : '<div class="empty">Nenhuma subcategoria encontrada na planilha.</div>';
}
...
function dadosTab(t, btn) {
  ...
  if (t==='sync') { updateSyncHero(); renderSyncSt(); renderPend(); updateDriveStatus();
    const u=document.getElementById('sh-url'); if(u) u.value=cfg.shUrl||'';
    const su=document.getElementById('sh-sheet-url'); if(su) su.value=cfg.sheetUrl||'';
    // Mostrar indicador de planilha detectada se já temos a URL
    const det=document.getElementById('sh-sheet-detected');
    if(det) det.style.display = cfg.sheetUrl ? 'flex' : 'none';
    // Status da URL
    const st=document.getElementById('sh-url-status');
    if(st && cfg.shUrl) st.innerHTML='<span style="color:var(--eme)">✓ Web App configurado</span>';
...
function populateFlt() {
  ...
  renderFltBlock('f-flt-cats',  _vivos('cat'),  'cats',  c=>`${c.emoji||''} ${c.nome}`, () => { populateFltSubs(); renderTrans(); });
}

function populateFltSubs() {
  const catIds = FLT.cats.length ? FLT.cats : _vivos('cat').map(c=>c.id);
  const mySubs = _vivosSubs(catIds);
...
function onCart(){
  // Cartão é independente — não força conta nem nada mais
  // Apenas sugere o tipo de pagamento conforme tipoUso do cartão
  ...
  if(!c){
    ...
    if(pagSel){pagSel.innerHTML='<option value="">Selecione...</option>'+_vivos('pag').map(p=>`<option value="${p.id}">${p.nome}</option>`).join('');}
    return;
  }
  // Sugerir pagamento com base no tipo do cartão
  if(c.tipoUso==='credit'){
    const px=pags.find(p=>p.tipo==='credit');
    if(px&&pagSel){pagSel.innerHTML=`<option value="${px.id}">${px.nome}</option>`;pagSel.value=px.id;}
    ...
    if(row)row.style.display='';
    if(pagSel){
      const opts=_vivos('pag').filter(p=>p.tipo==='credit'||p.tipo==='debit');
...
function editPag(id){
  ...
  document.querySelectorAll('.cfg-edit-panel.open').forEach(pp=>pp.classList.remove('open'));
  let panel=document.getElementById('edit-pag-'+id);
  if(!panel){
    const TIPOS=[{v:'credit',l:'Crédito'},{v:'debit',l:'Débito'},{v:'pix',l:'Pix'},{v:'dinheiro',l:'Dinheiro'},{v:'outro',l:'Outro'}];
    const tipoOpts=TIPOS.map(t=>`<option value="${t.v}"${t.v===p.tipo?' selected':''}>${t.l}</option>`).join('');
    panel=document.createElement('div');
    panel.id='edit-pag-'+id;
    panel.className='cfg-edit-panel';
    panel.innerHTML=`
      <h4>✏ Editar Pagamento</h4>
      <div class="frow"><div class="fg"><label>Nome</label><input id="ep-n-${id}" value="${p.nome.replace(/"/g,'&quot;')}"/></div></div>
      <div class="frow"><div class="fg"><label>Tipo</label><select id="ep-t-${id}">${tipoOpts}</select></div></div>
      <div style="display:flex;gap:8px;margin-top:4px">
        <button class="btn pr sm" style="flex:1" onclick="savePagEdit('${id}')">✅ Salvar</button>
        <button class="btn sm" onclick="this.closest('.cfg-edit-panel').classList.remove('open')">Cancelar</button>
      </div>`;
    const items=document.querySelectorAll('#pag-list .cfg-item');
    const target=Array.from(items).find(el=>el.querySelector(`button[onclick*="'${id}'"]`));
    if(target) target.after(panel); else document.getElementById('pag-list').appendChild(panel);
  }
  panel.classList.add('open');
  panel.scrollIntoView({behavior:'smooth',block:'nearest'});
}
...
function savePagEdit(id){
  if(_saveLock['savePag'+id])return; _saveLock['savePag'+id]=true;
  const p=pags.find(x=>x.id===id); if(!p){ delete _saveLock['savePag'+id]; return; }
  const n=document.getElementById('ep-n-'+id)||document.getElementById('ep-n');
  const t=document.getElementById('ep-t-'+id)||document.getElementById('ep-t');
  if(n) p.nome=n.value.trim()||p.nome;
  if(t) p.tipo=t.value||p.tipo;
  saveAll();
  const panel=document.getElementById('edit-pag-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  ...
  delete _saveLock['savePag'+id];
  if(cfg.shUrl&&navigator.onLine)saveAppData();
  toast('✅ Pagamento atualizado!');
}

// ── _getActivePage: usada por cfg-migrate.js e renderAll ──
// O filtro global completo (drawer, GFLT, chips) é definido e
// inicializado EXCLUSIVAMENTE em cfg-migrate.js — não duplicar
// aqui. Duplicar window.GFLT com nomes de campo diferentes
// ('per' vs 'modo') causa o app inteiro rodar sempre pelo
// caminho "filtro customizado" em vez do caminho simples padrão,
// mesmo sem nenhum filtro ativo, distorcendo o que aparece.
...
function saveCartEditInline(id){
  if(_saveLock['saveCartI'+id])return; _saveLock['saveCartI'+id]=true;
  const idx=carts.findIndex(x=>x.id===id);
  if(idx<0){delete _saveLock['saveCartI'+id];return;}
  const get=(sid)=>{ const el=document.getElementById(sid); return el?el.value:null; };
  const nm=get('ect-n-'+id); if(nm) carts[idx].nome=nm.trim()||carts[idx].nome;
  const cor=get('ect-cor-'+id); if(cor) carts[idx].cor=cor;
  const band=get('ect-band-'+id); if(band) carts[idx].band=band;
  const acc=get('ect-acc-'+id); if(acc) carts[idx].accId=acc;
  const lim=get('ect-lim-'+id); if(lim!==null) carts[idx].limite=parseMoeda(lim)||0;
  const fec=get('ect-fec-'+id); if(fec) carts[idx].fech=parseInt(fec)||10;
  const vec=get('ect-vec-'+id); if(vec) carts[idx].venc=parseInt(vec)||17;
  saveAll();
  const panel=document.getElementById('edit-cart-'+id);
  if(panel){panel.classList.remove('open');panel.remove();}
  renderCfgCartoes(); if(typeof renderCartList==='function') renderCartList();
  populateFlt(); populateForm();
  delete _saveLock['saveCartI'+id];
  if(cfg.shUrl&&navigator.onLine) saveAppData();
  toast('✅ Cartão atualizado!');
}
